...
7. `recommendedUpdateProfileCommand`, `recommendedUpdateProfileAndRefreshCommand`, and `recommendedInspectCommand` should stay aligned with the same target profile and ready-lane `paths` bundle including `profiles/<id>/profile.json`; if the work loop exposes metadata edits, include `profiles/<id>/profile.json` in that surfaced path set instead of leaving the edit target implicit.
