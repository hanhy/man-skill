...
Once that same already-imported profile has a runnable profile-local manifest, the top-level recommendation can switch to replaying imported intake directly via `recommendedCommand = node src/index.js import intake --person '<id>' --refresh-foundation` without dropping the inspect-first helper palette. `recommendedInspectCommand` still points at the plain `import intake --person '<id>'` replay, `recommendedUpdateProfileCommand` / `recommendedUpdateProfileAndRefreshCommand` stay attached to that same ready manifest, and `recommendedPaths` still keeps the concrete landing-zone files (`materials.template.json`, `sample.txt`, `images/chat.png`) visible so metadata edits and inspect-first replays remain obvious even after the manifest is already runnable.
