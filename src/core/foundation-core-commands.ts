...
function buildVoiceCommand(status: string | null): string | null {
  ...
  if (status === 'thin') {
    const normalizeToOpenclaw = [
      ...
      "    if (currentSection && currentSection !== 'current-default' && parsed.level > 2) { sections[currentSection].push(...parsed.rawLines); index += parsed.lineCount; continue; }",
...
function buildSoulCommand(status: string | null): string | null {
  ...
  if (status === 'thin') {
    const normalizeToOpenclaw = [
      ...
      "    if (currentSection && parsed.level > 2) { sections[currentSection].push(...parsed.rawLines); index += parsed.lineCount; continue; }",
