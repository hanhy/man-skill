...
function buildRecommendedImportedReadyProfileSlice(profile: any, rootDir: string | null) {
  if (!profile || typeof profile !== 'object') {
    return null;
  }

  const editPath = collectProfileMetadataEditPath(profile);
  const editPaths = editPath ? [editPath] : [];
  ...
  return {
    ...
    inspectCommand: normalizeOptionalString(profile?.importIntakeWithoutRefreshCommand),
    followUpCommand: null,
    paths: mergeUniquePaths(collectReadyIntakeImportPaths(profile, rootDir), editPaths),
  };
}
...
export function buildIngestionSummary(profiles: any[] = [], options: any = {}) {
  ...
  const helperCommands = {
    ...
    sampleStarter: sampleManifestPresent && sampleManifest.status === 'loaded'
      ...
      : null,
    sampleManifestInspect: sampleManifestPresent
  ...
  let recommendedUpdateProfileCommand: string | null = null;
  let recommendedUpdateProfileAndRefreshCommand: string | null = null;
  ...
  if (safeProfiles.length === 0) {
    ...
    recommendedUpdateProfileCommand = importedInvalidIntakeManifestProfiles.length > 1
      ? buildCommandBundle(importedInvalidIntakeManifestProfiles.map((profile) => profile?.updateProfileCommand ?? null))
      : (firstInvalidImportedIntakeProfile?.updateProfileCommand ?? null);
    recommendedUpdateProfileAndRefreshCommand = importedInvalidIntakeManifestProfiles.length > 1
      ? buildCommandBundle(importedInvalidIntakeManifestProfiles.map((profile) => profile?.updateProfileAndRefreshCommand ?? null))
      : (firstInvalidImportedIntakeProfile?.updateProfileAndRefreshCommand ?? null);
    ...
    recommendedUpdateProfileCommand = importedStarterIntakeProfiles.length > 1
      ? buildCommandBundle(importedStarterIntakeProfiles.map((profile) => profile?.updateProfileCommand ?? null))
      : (firstImportedStarterIntakeProfile?.updateProfileCommand ?? null);
    recommendedUpdateProfileAndRefreshCommand = importedStarterIntakeProfiles.length > 1
      ? buildCommandBundle(importedStarterIntakeProfiles.map((profile) => profile?.updateProfileAndRefreshCommand ?? null))
      : (firstImportedStarterIntakeProfile?.updateProfileAndRefreshCommand ?? null);
    ...
    recommendedCommand = importedProfilesWithReadyIntake.length > 1
      ...
      : (firstImportedReadyIntakeProfile?.importIntakeCommand
        ...
        ?? helperCommands.importIntakeImportedAndRefresh
        ?? null);
    recommendedInspectCommand = importedProfilesWithReadyIntake.length > 1
      ? (helperCommands.importIntakeImported
        ?? buildCommandBundle(importedProfilesWithReadyIntake.map((profile) => profile?.importIntakeWithoutRefreshCommand ?? null))
        ?? null)
    ...
    recommendedRefreshIntakeCommand = importedProfilesWithReadyIntake.length > 1
      ? buildCommandBundle(importedProfilesWithReadyIntake.map((profile) => profile?.updateIntakeCommand ?? null))
      : (firstImportedReadyIntakeProfile?.updateIntakeCommand ?? null);
    ...
    recommendedProfileSlices = importedProfilesWithReadyIntake
      .map((profile) => buildRecommendedImportedReadyProfileSlice(profile, rootDir))
      .filter((profile): profile is NonNullable<ReturnType<typeof buildRecommendedImportedReadyProfileSlice>> => Boolean(profile));
  ...
  return {
    ...
    recommendedUpdateProfileCommand,
    recommendedUpdateProfileAndRefreshCommand,
    recommendedEditPath,
    recommendedEditPaths,
