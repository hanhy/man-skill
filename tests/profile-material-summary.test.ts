...
test('PromptAssembler keeps latest material source paths visible in next-intake path hints without duplicating aliases', () => {
  const prompt = new PromptAssembler({
    ...
    ingestion: {
      ...
      recommendedManifestImportCommand: "node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json' --refresh-foundation",
...
test('buildIngestionSummary slash-normalizes starter-manifest edit paths before resolving template file targets', () => {
  const rootDir = makeTempRepo();
  const importsDir = path.join(rootDir, 'profiles', 'harry-han', 'imports');
  const imagesDir = path.join(importsDir, 'images');
  fs.mkdirSync(imagesDir, { recursive: true });
  fs.writeFileSync(path.join(importsDir, 'sample.txt'), 'Ship the first slice before polishing the plan.\n');
  fs.writeFileSync(path.join(imagesDir, 'chat.png'), 'png-placeholder');
  fs.writeFileSync(path.join(importsDir, 'materials.template.json'), JSON.stringify({
    ...
    entryTemplates: {
      ...
      screenshot: { type: 'screenshot', file: 'images\\chat.png' },
  ...
  assert.deepEqual(jsSummary, tsSummary);
  assert.equal(tsSummary.recommendedEditPath, 'profiles/harry-han/imports/materials.template.json');
...
test('loadProfilesIndex marks memory foundation drafts stale when stored entry summaries drift from imported materials', () => {
  const rootDir = makeTempRepo();
  const ingestion = new MaterialIngestion(rootDir);
  const loader = new FileSystemLoader(rootDir);
  ...
  assert.deepEqual(profile.foundationDraftStatus.refreshReasons, ['draft metadata drift']);
  assert.equal(profile.foundationDraftSummaries.memory.generated, true);
  assert.deepEqual(profile.foundationDraftSummaries.memory.latestSummaries, ['Old summary that no longer matches the imported material.']);
});
...
test('PromptAssembler includes delivery foundation snapshots in the system prompt', () => {
  ...
  assert.match(prompt, /Jane Doe \(jane-doe\): 1 material \(talk:1\), latest 2026-04-16T16:00:00\.000Z \| refresh node src\/index\.js update foundation --person 'jane-doe' \| update node src\/index\.js update profile --person 'jane-doe' --display-name 'Jane Doe' \| sync node src\/index\.js update profile --person 'jane-doe' --display-name 'Jane Doe' --refresh-foundation/);
...
test('PromptAssembler falls back to top-level ingestion starter bundle mirrors for helper lines', () => {
  const prompt = new PromptAssembler({
    profile: { name: 'ManSkill', soul: 'persona core', identity: {} },
    voice: { style: 'direct' },
    memory: { shortTermEntries: 0, longTermEntries: 0 },
...
test('PromptAssembler keeps the ingestion entrance visible when only the manifest refresh shortcut is available', () => {
  ...
  assert.match(prompt, /Ingestion entrance:/);
  assert.match(prompt, /helpers: .*manifest-inspect node src\/index\.js import manifest --file <manifest\.json>.*manifest node src\/index\.js import manifest --file <manifest\.json> --refresh-foundation/);
...
test('PromptAssembler slash-normalizes recommended starter profile edit paths before rendering prompt summaries', () => {
  const prompt = new PromptAssembler({
    profile: { name: 'ManSkill', soul: 'persona core', identity: {} },
    voice: { style: 'direct' },
    memory: { shortTermEntries: 0, longTermEntries: 0 },
    ...
    workLoop: {
      intervalMinutes: 10,
      objectiveCount: 1,
      objectives: ['improve the user-facing ingestion/update entrance for target-person materials'],
      priorityCount: 2,
      readyPriorityCount: 1,
      ...
      runnablePriority: {
        ...
        status: 'ready',
        summary: 'starter scaffold available',
        nextAction: 'populate imported intake starter manifests — starting with Harry Han (harry-han)',
        command: "(node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json') && (node src/index.js import manifest --file 'profiles/jane-doe/imports/materials.template.json')",
        recommendedProfileSlices: [
          ...
          },
          {
            ...
            updateProfileAndRefreshCommand: null,
            editPath: null,
            editPaths: [' .\\profiles\\jane-doe//imports\\materials.template.json '],
            manifestInspectCommand: null,
            manifestImportCommand: null,
            intakeManifestEntryTemplateTypes: [],
            intakeManifestEntryTemplateDetails: [],
          ...
          {
            personId: 'second-pass',
            label: 'Second Pass (second-pass)',
            latestMaterialAt: null,
            latestMaterialId: null,
            latestMaterialSourcePath: null,
            candidateSignalSummary: null,
            ...
            inspectCommand: null,
            followUpCommand: null,
            paths: [' .\\profiles\\second-pass//imports\\materials.template.json '],
          },
        ],
        paths: [
          'profiles/harry-han/imports/materials.template.json',
  ...
  assert.match(prompt, /recommended starter profiles: Harry Han \(harry-han\) -> profiles\/harry-han\/imports\/materials\.template\.json \| Jane Doe \(jane-doe\) -> profiles\/jane-doe\/imports\/materials\.template\.json \| Second Pass \(second-pass\) -> profiles\/second-pass\/imports\/materials\.template\.json/);
  assert.match(prompt, /runnable starter profiles: Harry Han \(harry-han\) -> profiles\/harry-han\/imports\/materials\.template\.json \| Jane Doe \(jane-doe\) -> profiles\/jane-doe\/imports\/materials\.template\.json \| Second Pass \(second-pass\) -> profiles\/second-pass\/imports\/materials\.template\.json/);
...
test('buildSummary exposes an ingestion entrance rollup with actionable commands', () => {
  ...
  assert.equal(summary.ingestion.importManifestInspectCommand, 'node src/index.js import manifest --file <manifest.json>');
  ...
  assert.deepEqual(metadataOnlyCommand, {
    ...
    intakeManifestEntryTemplateDetails: [],
    intakeManifestEntryTemplateCount: 0,
    intakeManifestEntryTemplateRoot: 'profiles/metadata-only/imports',
    intakePaths: [
      'profiles/metadata-only/imports',
  ...
  assert.match(summary.promptPreview, /next intake: refresh stale or incomplete target profiles; command node src\/index\.js update foundation --person 'jane-doe'; latest material \d{4}-\d{2}-\d{2}T[^ ]+ \([^)]*\)(?: @ [^;]+)?; evidence memory 1 \(talk\) \| voice 1 \(talk\) \| soul 1 \(talk\) \| skills 1 \(talk\); draft sources [^\n;]+; gaps memory missing, 1 candidate \(Tight loops beat big plans\.\) \| skills 1\/3 ready \(candidate-skills\), missing evidence\/gaps-to-validate \| soul 2\/4 ready \(core-truths, boundaries\), missing vibe\/continuity; aliases core-values->core-truths \| voice 1\/4 ready \(tone\), missing signature-moves\/avoid\/language-hints; paths profiles\/jane-doe\/memory\/long-term\/foundation\.json, profiles\/jane-doe\/skills\/README\.md, profiles\/jane-doe\/soul\/README\.md, profiles\/jane-doe\/voice\/README\.md/);
  ...
  assert.deepEqual(summary.ingestion.recommendedProfileSlices, [
    {
      ...
      intakeManifestEntryTemplateRoot: harryCommand?.intakeManifestEntryTemplateRoot ?? null,
    ...
    },
    {
      ...
      intakeManifestEntryTemplateRoot: janeCommand?.intakeManifestEntryTemplateRoot ?? null,
...
test('buildSummary keeps imported profiles with invalid intake manifests in the actionable ingestion queue', () => {
  ...
  assert.match(summary.promptPreview, /next intake: repair the invalid intake manifest for imported profile Harry Han \(harry-han\) — invalid JSON; command node src\/index\.js update intake --person 'harry-han' --display-name 'Harry Han'(?: --summary '.*')?; latest material \d{4}-\d{2}-\d{2}T[^;]+; evidence memory 1 \(message\) \| voice 1 \(message\) \| soul 0 \| skills 0; draft sources [^\n;]+; edit profiles\/harry-han\/imports\/materials\.template\.json; refresh intake node src\/index\.js update intake --person 'harry-han' --display-name 'Harry Han'(?: --summary '.*')?; update profile node src\/index\.js update profile --person 'harry-han' --display-name 'Harry Han'(?: --summary '.*')?; sync profile node src\/index\.js update profile --person 'harry-han' --display-name 'Harry Han'(?: --summary '.*')? --refresh-foundation; starter root profiles\/harry-han\/imports; manifest inspect node src\/index\.js import manifest --file 'profiles\/harry-han\/imports\/materials\.template\.json'; manifest node src\/index\.js import manifest --file 'profiles\/harry-han\/imports\/materials\.template\.json' --refresh-foundation; inspect after editing node src\/index\.js import intake --person 'harry-han'; then run node src\/index\.js import intake --person 'harry-han' --refresh-foundation; paths profiles\/harry-han\/imports\/materials\.template\.json/);
...
test('buildSummary treats metadata-only intake manifest symlinks that escape the repo root as invalid', () => {
  ...
  fs.rmSync(path.join(rootDir, 'profiles', 'metadata-only', 'imports', 'materials.template.json'));
  fs.symlinkSync(
    path.join(outsideDir, 'outside-manifest.json'),
    path.join(rootDir, 'profiles', 'metadata-only', 'imports', 'materials.template.json'),
  );
...
test('buildSummary keeps the ingestion entrance visible for empty repos', () => {
  ...
  assert.deepEqual(summary.ingestion, {
    ...
    recommendedDraftSourcesSummary: null,
    recommendedDraftGapSummary: null,
    ...
    recommendedIntakeManifestEntryTemplateRoot: null,
    recommendedProfileSlices: [],
...
test('buildSummary treats sample manifests with missing referenced files as invalid and hides broken starter commands', () => {
  ...
  assert.match(summary.promptPreview, /next intake: fix the checked-in sample manifest for Harry Han \(harry-han\) — missing file: missing-post\.txt; manifest inspect node src\/index\.js import manifest --file 'samples\/harry-materials\.json'; paths samples\/harry-materials\.json, samples\/harry-post\.txt, samples\/missing-post\.txt/);
  assert.doesNotMatch(summary.promptPreview, /starter: node src\/index\.js import sample/);
});
