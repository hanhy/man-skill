...
test('PromptAssembler includes delivery foundation snapshots in the system prompt', () => {
  ...
  assert.match(prompt, /Metadata Only \(metadata-only\): 0 materials \(no typed materials\); import node src\/index\.js import text --person metadata-only --file <sample\.txt> --refresh-foundation \| update node src\/index\.js update profile --person metadata-only/);
...
test('buildSummary shell-quotes sample ingestion commands when discovered sample paths contain spaces', () => {
  ...
  assert.match(summary.promptPreview, /sample manifest: 1 entry for harry-han -> node src\/index\.js import manifest --file 'samples\/harry sample materials\.json' --refresh-foundation/);
