...
test('buildSummary keeps delivery queues aligned with the canonical rollout order', () => {
  ...
  assert.deepEqual(summary.skills.foundationStatusCounts, { ready: 1 });
...
test('buildSummary reports fully active delivery readiness even when channel and provider queues are empty', () => {
  const rootDir = makeTempRepo();
  seedMinimalRepo(rootDir);
  ...
  assert.equal(summary.delivery.pendingChannelCount, 0);
  assert.equal(summary.delivery.pendingProviderCount, 0);
  ...
  assert.deepEqual(summary.delivery.channelQueue, []);
...
test('buildSummary counts the checked-in channel delivery modules and all provider helpers as runtime-ready', () => {
  ...
  assert.match(summary.promptPreview, /\+2 more channels: WhatsApp \[candidate, runtime-ready\], Slack \[candidate, runtime-ready\]/);
  assert.match(summary.promptPreview, /\+3 more queued channels: Telegram \[candidate, runtime-ready\], WhatsApp \[candidate, runtime-ready\], Slack \[candidate, runtime-ready\]/);
...
test('buildSummary exposes a delivery setup queue and prompt preview includes setup hints', () => {
  ...
  try {
    ...
    assert.deepEqual(summary.delivery.channelQueue[0], {
      ...
      implementationScaffoldPath: 'src/channels/feishu.js',
      configured: false,
      missingEnvVars: ['FEISHU_APP_ID', 'FEISHU_APP_SECRET'],
...
test('buildSummary prompt preview surfaces delivery implementation bundles when multiple scaffolds are missing', () => {
  ...
  assert.equal(
    ...
    "(mkdir -p 'src/channels' && touch 'src/channels/telegram.js') && (mkdir -p 'src/channels' && touch 'src/channels/slack.js')",
  );
  assert.equal(
  ...
  assert.match(summary.promptPreview, /channel impl-all \(mkdir -p 'src\/channels' && touch 'src\/channels\/telegram\.js'\) && \(mkdir -p 'src\/channels' && touch 'src\/channels\/slack\.js'\)/);
