...
test('architecture and ingestion docs explain work-loop leader/blocker semantics and sample-manifest entrypoints', () => {
  ...
  assert.match(architectureDoc, /additive follow-up metadata \(`fallbackCommand`, `editPath`, `editPaths`, `inspectCommand`, `followUpCommand`\)/i);
  assert.match(architectureDoc, /prompt-preview `Work loop:` block also serializes the same `fallback`, `edit` \/ `edit paths`, `inspect after editing`, and `then run` surfaces/i);
...
test('checked-in intake scaffold stays aligned with the repo-level starter ingress for Harry Han', () => {
  ...
  assert.match(readme, /Once an already-imported profile's drafts are fresh and it still has only the untouched `profiles\/<person-id>\/imports\/materials\.template\.json` starter manifest, the ingestion entrance keeps the top-level `next intake` step descriptive while also surfacing `recommendedEditPath`, `recommendedEditPaths`, `recommendedInspectCommand`, `recommendedFollowUpCommand`, and `recommendedFallbackCommand`, so operators know which file to edit in the single-profile case, which manifest set to edit in the bundled multi-profile case, which plain `import intake --person <id>` inspection replay to run first, which `import intake --person <id> --refresh-foundation` replay to run next, and which direct starter import still works as a fallback/i);
  ...
  assert.match(ingestionDoc, /recommendedEditPath`.*`recommendedEditPaths`.*`recommendedInspectCommand`.*`recommendedFollowUpCommand`/i);
  ...
  assert.match(profilesDoc, /top-level recommendation stays edit-first: `recommendedCommand` remains empty while `recommendedEditPath` points at the manifest for a single starter-template profile, `recommendedEditPaths` expands that into a bundled manifest set when multiple imported starter templates are queued, `recommendedInspectCommand` shows the plain `import intake --person <id>` inspection replay to run right after the edit, `recommendedFollowUpCommand` shows the `import intake --person <id> --refresh-foundation` replay to run after that, and `recommendedFallbackCommand` keeps the seeded direct starter import visible when operators want a stopgap import before customizing the manifest/i);
...
test('repo memory, skills, soul, and voice docs stay aligned with the structured foundation sections', () => {
  ...
  assert.match(readme, /top-level `Skill registry:` block now mirrors both as `- categories: \.\.\.` and `- documented categories: \.\.\.`/i);
  ...
  assert.match(architectureDoc, /compact `Skill registry:` preview should mirror both counts as `- categories: \.\.\.` and `- documented categories: \.\.\.`/i);
  ...
  assert.match(readme, /`buildSummary\(\.\.\.\)`.*top-level `profileSnapshots\[\]` records \(`id`, `label`, `snapshot`, `lines`, `draftFiles`, `draftGaps`, and `highlights`\)/i);
  assert.match(architectureDoc, /top-level `buildSummary\(\.\.\.\)\.profileSnapshots\[\]` records \(`id`, `label`, `snapshot`, `lines`, `draftFiles`, `draftGaps`, `highlights`\)/i);
  assert.match(ingestionDoc, /`buildSummary\(\.\.\.\)\.profileSnapshots\[\]` mirrors the compact operator-facing profile foundation snapshot surface as machine-readable records \(`id`, `label`, `snapshot`, `lines`, `draftFiles`, `draftGaps`, `highlights`\)/i);
