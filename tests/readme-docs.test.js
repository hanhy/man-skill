...
test('architecture and ingestion docs explain work-loop leader/blocker semantics and sample-manifest entrypoints', () => {
  ...
  assert.match(architectureDoc, /additive follow-up metadata \(`latestMaterialAt`, `latestMaterialId`, `latestMaterialSourcePath`, `refreshReasons`, `missingDrafts`, `candidateSignalSummary`, `draftGapSummary`, `fallbackCommand`, `refreshIntakeCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, `followUpCommand`\)/i);
  assert.match(architectureDoc, /prompt-preview `Work loop:` block also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft gaps`, `fallback`, `refresh intake`, `edit` \/ `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces/i);
  ...
  assert.match(ingestionDoc, /each `workLoop` priority row can now also carry additive follow-up metadata \(`latestMaterialAt`, `latestMaterialId`, `latestMaterialSourcePath`, `refreshReasons`, `missingDrafts`, `candidateSignalSummary`, `draftGapSummary`, `fallbackCommand`, `refreshIntakeCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, `followUpCommand`\)/i);
  assert.match(ingestionDoc, /prompt-preview `Work loop:` block now also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft gaps`, `fallback`, `refresh intake`, `edit` \/ `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces/i);
  assert.match(readme, /each priority can carry additive follow-up surfaces like `latestMaterialAt`, `latestMaterialId`, `latestMaterialSourcePath`, `refreshReasons`, `missingDrafts`, `candidateSignalSummary`, `draftGapSummary`, `fallbackCommand`, `refreshIntakeCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, and `followUpCommand`/i);
  assert.match(readme, /explicit `latest material:`, `refresh reasons:`, `missing drafts:`, `evidence:`, `draft gaps:`, `fallback:`, `refresh intake:`, `edit:`, `edit paths:`, `manifest inspect:`, `manifest:`, `inspect after editing:`, and `then run:` lines/i);
  assert.match(profilesDoc, /same contract now propagates into `workLoop\.currentPriority\|actionableReadyPriority\|recommendedPriority` through `latestMaterialAt`, `latestMaterialId`, `latestMaterialSourcePath`, `refreshReasons`, `missingDrafts`, `candidateSignalSummary`, `draftGapSummary`, `fallbackCommand`, `refreshIntakeCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, and `followUpCommand`/i);
  ...
  assert.match(ingestionDoc, /blocked delivery priorities keep their exact env\/bootstrap command while keeping bootstrap `paths` on `\.env\.example, \.env`/);
...
test('checked-in intake scaffold stays aligned with the repo-level starter ingress for Harry Han', () => {
  ...
  assert.match(readme, /Once an already-imported profile's drafts are fresh and it still has only the untouched `profiles\/<person-id>\/imports\/materials\.template\.json` starter manifest, the ingestion entrance keeps the top-level `next intake` step descriptive while also surfacing `recommendedRefreshIntakeCommand`, `recommendedEditPath`, `recommendedEditPaths`, `recommendedManifestInspectCommand`, `recommendedManifestImportCommand`, `recommendedIntakeManifestEntryTemplateTypes`, `recommendedIntakeManifestEntryTemplateCount`, `recommendedInspectCommand`, `recommendedFollowUpCommand`, and `recommendedFallbackCommand`, so operators know which intake scaffold rerun restores the profile-local landing zone before import, which file to edit first in the single-profile case, which broader starter-edit surface to touch in bundled or file-backed cases .*`sample\.txt`.*`images\/chat\.png`.*, which starter template entry types are still expected before import, which plain `import intake --person <id>` inspection replay to run first, which `import intake --person <id> --refresh-foundation` replay to run next, and which direct starter import is still available as a stopgap/i);
