...
test('checked-in intake scaffold stays aligned with the repo-level starter ingress for Harry Han', () => {
  ...
  assert.match(ingestionDoc, /once that same already-imported profile has a runnable profile-local manifest, the recommendation can promote `recommendedCommand` to `node src\/index\.js import intake --person '<id>' --refresh-foundation` while still preserving `recommendedInspectCommand`, `recommendedUpdateProfileCommand`, `recommendedUpdateProfileAndRefreshCommand`, and the same `recommendedPaths` landing-zone bundle .*`materials\.template\.json`.*`sample\.txt`.*`images\/chat\.png`/i);
  ...
  assert.match(profilesDoc, /Once that same already-imported profile has a runnable profile-local manifest, the top-level recommendation can switch to replaying imported intake directly via `recommendedCommand = node src\/index\.js import intake --person '<id>' --refresh-foundation` without dropping the inspect-first helper palette.*`recommendedInspectCommand` still points at the plain `import intake --person '<id>'` replay.*`recommendedUpdateProfileCommand` \/ `recommendedUpdateProfileAndRefreshCommand` stay attached to that same ready manifest.*`recommendedPaths` still keeps the concrete landing-zone files .*`materials\.template\.json`.*`sample\.txt`.*`images\/chat\.png`/i);
