...
test('architecture and ingestion docs explain work-loop leader/blocker semantics and sample-manifest entrypoints', () => {
  ...
  assert.match(architectureDoc, /additive follow-up metadata \(`fallbackCommand`, `refreshIntakeCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, `followUpCommand`\)/i);
  assert.match(architectureDoc, /prompt-preview `Work loop:` block also serializes the same `fallback`, `refresh intake`, `edit` \/ `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces/i);
  ...
  assert.match(ingestionDoc, /each `workLoop` priority row can now also carry additive follow-up metadata \(`fallbackCommand`, `refreshIntakeCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, `followUpCommand`\)/i);
  assert.match(ingestionDoc, /prompt-preview `Work loop:` block now also serializes the same `fallback`, `refresh intake`, `edit` \/ `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces/i);
  assert.match(readme, /each priority can carry additive follow-up surfaces like `fallbackCommand`, `refreshIntakeCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, and `followUpCommand`/i);
  assert.match(readme, /explicit `fallback:`, `refresh intake:`, `edit:`, `edit paths:`, `manifest inspect:`, `manifest:`, `inspect after editing:`, and `then run:` lines/i);
