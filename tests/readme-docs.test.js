...
const profileUpdateSkill = fs.readFileSync(path.join(repoRoot, 'skills', 'profiles', 'update', 'SKILL.md'), 'utf8');
    ...
    .map((key) => {
      ...
      const sourceLabel = sourceCount > 0
        ...
        : materialTypes
          ? `(${materialTypes})`
          : null;
      const latestMaterialSourcePath = typeof source.latestMaterialSourcePath === 'string' && source.latestMaterialSourcePath.length > 0
        ? source.latestMaterialSourcePath
        : null;
      const detailParts = [
        sourceLabel,
...
test('README documents the default delivery foundation targets and repo manifests', () => {
  ...
  assert.match(readme, /`recommended fallback:`, `recommended refresh intake:`, `recommended update profile:`, `recommended sync profile:`, `recommended edit:`, `recommended edit paths:`, `recommended manifest inspect:`, `recommended manifest:`, `recommended inspect first\|after editing:`, `recommended then run:`, and `recommended paths:` lines/i);
...
test('architecture and ingestion docs explain work-loop leader/blocker semantics and sample-manifest entrypoints', () => {
  ...
  assert.match(architectureDoc, /exact checked-in sample manifest inspect command via `sampleManifestInspectCommand`/);
  assert.match(architectureDoc, /exact checked-in sample manifest command via `sampleManifestCommand`/);
  assert.match(architectureDoc, /generic inspect-first manifest helper via `importManifestInspectCommand`/);
  ...
  assert.match(architectureDoc, /prompt-preview `Work loop:` block also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft sources`, `draft gaps`, `fallback`, `refresh intake`, `update profile`, `sync profile`, `edit` \/ `edit paths`, `manifest inspect`, `manifest`, `inspect first\|after editing`, `then run`, and `paths` surfaces/i);
  ...
  assert.match(ingestionDoc, /prompt-preview `Work loop:` block now also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft sources`, `draft gaps`, `fallback`, `refresh intake`, `update profile`, `sync profile`, `edit` \/ `edit paths`, `manifest inspect`, `manifest`, `inspect first\|after editing`, `then run`, and `paths` surfaces/i);
  assert.match(ingestionDoc, /`recommendedPriority` points at a ready edit-first ingestion slice instead of the current blocker, the same `Work loop:` block now keeps `recommended fallback`, `recommended refresh intake`, `recommended update profile`, `recommended sync profile`, `recommended edit` \/ `recommended edit paths`, `recommended manifest inspect`, `recommended manifest`, `recommended inspect first\|after editing`, `recommended then run`, and `recommended paths` visible/i);
  assert.match(readme, /each priority can carry additive follow-up surfaces like `latestMaterialAt`, `latestMaterialId`, `latestMaterialSourcePath`, `refreshReasons`, `missingDrafts`, `candidateSignalSummary`, `draftSourcesSummary`, `draftGapSummary`, `fallbackCommand`, `refreshIntakeCommand`, `updateProfileCommand`, `updateProfileAndRefreshCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateDetails`, `intakeManifestEntryTemplateCount`, `intakeManifestEntryTemplateRoot`, `inspectCommand`, and `followUpCommand`/i);
  assert.match(readme, /prompt preview mirrors that distinction with separate `lead:`, `recommended:`, `current:`, `runnable:`, and `advisory:` lines when they differ/i);
  assert.match(readme, /when the recommended lane is not the current blocker it also surfaces explicit `recommended fallback:`, `recommended refresh intake:`, `recommended update profile:`, `recommended sync profile:`, `recommended edit:`, `recommended edit paths:`, `recommended manifest inspect:`, `recommended manifest:`, `recommended inspect first\|after editing:`, `recommended then run:`, and `recommended paths:` lines/i);
  assert.match(profilesDoc, /same contract now propagates into `workLoop\.currentPriority\|actionableReadyPriority\|recommendedPriority` through `latestMaterialAt`, `latestMaterialId`, `latestMaterialSourcePath`, `refreshReasons`, `missingDrafts`, `candidateSignalSummary`, `draftSourcesSummary`, `draftGapSummary`, `fallbackCommand`, `refreshIntakeCommand`, `updateProfileCommand`, `updateProfileAndRefreshCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateDetails`, `intakeManifestEntryTemplateCount`, `intakeManifestEntryTemplateRoot`, `inspectCommand`, and `followUpCommand`/i);
...
test('checked-in intake scaffold stays aligned with the repo-level starter ingress for Harry Han', () => {
  ...
  assert.match(readme, /Once that same already-imported profile has a runnable profile-local manifest, the recommendation can promote `recommendedCommand` to `node src\/index\.js import intake --person '<id>' --refresh-foundation` while still keeping `recommendedInspectCommand` on the plain `import intake --person '<id>'` replay, preserving `recommendedUpdateProfileCommand` \/ `recommendedUpdateProfileAndRefreshCommand` beside the ready replay, and carrying the same `recommendedPaths` bundle .*`materials\.template\.json`.*`sample\.txt`.*`images\/chat\.png`.*`profiles\/<id>\/profile\.json`/i);
  ...
  assert.match(ingestionDoc, /once that same already-imported profile has a runnable profile-local manifest, the recommendation can promote `recommendedCommand` to `node src\/index\.js import intake --person '<id>' --refresh-foundation` while still preserving `recommendedInspectCommand`, `recommendedUpdateProfileCommand`, `recommendedUpdateProfileAndRefreshCommand`, and the same `recommendedPaths` landing-zone bundle .*`materials\.template\.json`.*`sample\.txt`.*`images\/chat\.png`.*`profiles\/<id>\/profile\.json`/i);
  ...
  assert.match(profilesDoc, /top-level recommendation stays edit-first but now keeps `recommendedCommand` pointed at the inspect-first `import manifest --file 'profiles\/<id>\/imports\/materials\.template\.json'` step while `recommendedLatestMaterialAt` \/ `recommendedLatestMaterialId` \/ `recommendedLatestMaterialSourcePath` keep the newest imported artifact attached to that follow-up, `recommendedRefreshIntakeCommand` points at the `update intake` rerun that restores the profile-local scaffold, `recommendedUpdateProfileCommand` and `recommendedUpdateProfileAndRefreshCommand` keep plain metadata edits and metadata-sync-with-refresh visible beside that manifest work, `recommendedEditPath` points at the manifest for a single starter-template profile, `recommendedEditPaths` expands beyond that primary manifest into the full starter-edit surface .*`sample\.txt`.*`images\/chat\.png`.*`recommendedManifestInspectCommand` \/ `recommendedManifestImportCommand` keep the direct `import manifest --file 'profiles\/<id>\/imports\/materials\.template\.json'` inspect\/refresh commands visible during the edit, `recommendedIntakeManifestEntryTemplateTypes` \/ `recommendedIntakeManifestEntryTemplateDetails` \/ `recommendedIntakeManifestEntryTemplateCount` preserve which starter entry types and concrete placeholder details are still expected before import, `recommendedIntakeManifestEntryTemplateRoot` keeps the shared `profiles\/<id>\/imports` landing-zone root explicit for those relative starter file paths, `recommendedProfileSlices` keeps the per-profile `label -> edit path` mapping machine-readable when more than one imported starter manifest is queued while also mirroring each slice's `refreshReasons`, `missingDrafts`, and `draftGapSummary`, `recommendedInspectCommand` shows the plain `import intake --person <id>` inspection replay to run right after the edit, `recommendedFollowUpCommand` shows the `import intake --person <id> --refresh-foundation` replay to run after that, and `recommendedFallbackCommand` keeps the seeded direct starter import visible as the final escape hatch/i);
  assert.match(profilesDoc, /Once that same already-imported profile has a runnable profile-local manifest, the top-level recommendation can switch to replaying imported intake directly via `recommendedCommand = node src\/index\.js import intake --person '<id>' --refresh-foundation` without dropping the inspect-first helper palette.*`recommendedInspectCommand` still points at the plain `import intake --person '<id>'` replay.*`recommendedUpdateProfileCommand` \/ `recommendedUpdateProfileAndRefreshCommand` stay attached to that same ready manifest.*`recommendedPaths` still keeps the concrete landing-zone files .*`materials\.template\.json`.*`sample\.txt`.*`images\/chat\.png`.*`profiles\/<id>\/profile\.json`/i);
  ...
  assert.match(profilesDoc, /generated `profiles\/<id>\/imports\/README\.md` also carries a `Rerun safety:` block that says re-running `update intake` preserves starter entries, entry templates, and the managed `Custom notes` block while snapshotting invalid JSON starter manifests to `materials\.template\.json\.invalid-<timestamp>\.bak` before rebuilding/i);
  ...
  assert.match(architectureDoc, /once an imported profile-local manifest is already runnable, keep the ready replay on `recommendedCommand = import intake --person <id> --refresh-foundation` but still preserve the plain inspection replay through `recommendedInspectCommand` \/ `inspectCommand`, alongside the same metadata-edit helpers and landing-zone paths/i);
  ...
  assert.match(harryIntakeReadme, /(?:Profile-local|Starter) manifest: profiles\/harry-han\/imports\/materials\.template\.json/);
  ...
  assert.match(harryIntakeReadme, /4\. Once the inspection looks right, run the import command above to ingest materials and refresh foundation drafts\./);
  ...
  assert.match(harryIntakeReadme, /outside paths or symlinks that escape the repo root are rejected during import/i);
  ...
  assert.match(harryIntakeReadme, /Rerun safety:/);
  assert.match(harryIntakeReadme, /re-running `node src\/index\.js update intake --person 'harry-han' --display-name 'Harry Han' --summary 'Direct operator with a bias for momentum and fast feedback loops\.'` preserves starter entries, entry templates, and the managed Custom notes block/i);
  assert.match(harryIntakeReadme, /if `materials\.template\.json` becomes invalid JSON, the same rerun snapshots it to `profiles\/harry-han\/imports\/materials\.template\.json\.invalid-<timestamp>\.bak` before rebuilding the starter scaffold/i);
  ...
  if (harryIntakeManifest.entries.length > 0) {
    assert.match(harryIntakeReadme, /import via the profile-local intake shortcut without refreshing drafts: node src\/index\.js import intake --person 'harry-han'/);
    assert.match(harryIntakeReadme, /import via the profile-local intake shortcut and refresh drafts: node src\/index\.js import intake --person 'harry-han' --refresh-foundation/);
    assert.doesNotMatch(harryIntakeReadme, /after editing, replay the profile-local intake without refreshing drafts:/);
    assert.doesNotMatch(harryIntakeReadme, /after editing, replay the profile-local intake and refresh drafts:/);
  } else {
  ...
  assert.ok(Array.isArray(harryIntakeManifest.entries));
  ...
  if (harryIntakeManifest.entries.length === 0) {
    ...
    assert.match(harryIntakeSample, /\S/);
  }
  assert.equal(harryIntakeScreenshot.subarray(0, 8).toString('hex'), '89504e470d0a1a0a');

  const summary = buildSummary(repoRoot);
  const profileSnapshot = summary.profileSnapshots[0];
  const profileSummary = summary.profiles[0];
  const harryCommand = summary.ingestion.allProfileCommands.find((profile) => profile.personId === 'harry-han');

  assert.equal(summary.profileSnapshots.length, 1);
  assert.ok(harryCommand);
  assert.match(profileSnapshot.label, /^Harry Han \(harry-han\)$/);
  assert.equal(
    profileSnapshot.materialCount,
    Object.values(profileSnapshot.materialTypes).reduce((total, count) => total + count, 0),
  );
  assert.match(
    profileSnapshot.snapshot,
    new RegExp(`^- Harry Han \\(harry-han\\): ${profileSnapshot.materialCount} materials \\(${escapeRegExp(formatMaterialTypes(profileSnapshot.materialTypes))}\\)`, 'm'),
  );
  assert.equal(profileSnapshot.latestMaterialId, profileSummary.latestMaterialId);
  assert.equal(profileSnapshot.latestMaterialSourcePath, profileSummary.latestMaterialSourcePath);
  assert.deepEqual(profileSnapshot.draftStatus, {
    generatedAt: profileSummary.foundationDraftStatus.generatedAt,
    complete: profileSummary.foundationDraftStatus.complete,
    needsRefresh: profileSummary.foundationDraftStatus.needsRefresh,
    missingDrafts: profileSummary.foundationDraftStatus.missingDrafts,
    refreshReasons: profileSummary.foundationDraftStatus.refreshReasons,
  });
  for (const area of ['memory', 'voice', 'soul', 'skills']) {
    assert.equal(profileSnapshot.readiness[area].candidateCount, profileSummary.foundationReadiness[area].candidateCount);
    const snapshotSampleTypes = [...new Set(profileSnapshot.readiness[area].sampleTypes ?? [])].sort();
    const summarySampleTypes = [...new Set(profileSummary.foundationReadiness[area].sampleTypes ?? [])].sort();
    assert.deepEqual(snapshotSampleTypes, summarySampleTypes);
  }
  assert.match(
    profileSnapshot.snapshot,
    new RegExp(
      escapeRegExp(
        `memory candidates: ${profileSnapshot.readiness.memory.candidateCount}`
      ),
    ),
  );
  if (profileSnapshot.refreshCommand) {
    assert.match(profileSnapshot.refreshCommand, /^node src\/index\.js update foundation --person 'harry-han'$/);
    assert.ok(profileSnapshot.refreshPaths.length > 0);
  } else {
    assert.deepEqual(profileSnapshot.refreshPaths, []);
  }
  assert.deepEqual(profileSnapshot.draftSources, {
    memory: {
      path: profileSummary.foundationDraftSummaries.memory.path,
      generated: true,
      generatedAt: profileSummary.foundationDraftSummaries.memory.generatedAt,
      latestMaterialAt: profileSummary.foundationDraftSummaries.memory.latestMaterialAt,
      latestMaterialId: profileSummary.foundationDraftSummaries.memory.latestMaterialId,
      ...(profileSummary.foundationDraftSummaries.memory.latestMaterialSourcePath
        ? { latestMaterialSourcePath: profileSummary.foundationDraftSummaries.memory.latestMaterialSourcePath }
        : {}),
      sourceCount: profileSummary.foundationDraftSummaries.memory.sourceCount,
      materialTypes: profileSummary.foundationDraftSummaries.memory.materialTypes,
      entryCount: profileSummary.foundationDraftSummaries.memory.entryCount,
    },
    skills: {
      path: profileSummary.foundationDraftSummaries.skills.path,
      generated: true,
      generatedAt: profileSummary.foundationDraftSummaries.skills.generatedAt,
      latestMaterialAt: profileSummary.foundationDraftSummaries.skills.latestMaterialAt,
      latestMaterialId: profileSummary.foundationDraftSummaries.skills.latestMaterialId,
      ...(profileSummary.foundationDraftSummaries.skills.latestMaterialSourcePath
        ? { latestMaterialSourcePath: profileSummary.foundationDraftSummaries.skills.latestMaterialSourcePath }
        : {}),
      sourceCount: profileSummary.foundationDraftSummaries.skills.sourceCount,
      materialTypes: profileSummary.foundationDraftSummaries.skills.materialTypes,
    },
    soul: {
      path: profileSummary.foundationDraftSummaries.soul.path,
      generated: true,
      generatedAt: profileSummary.foundationDraftSummaries.soul.generatedAt,
      latestMaterialAt: profileSummary.foundationDraftSummaries.soul.latestMaterialAt,
      latestMaterialId: profileSummary.foundationDraftSummaries.soul.latestMaterialId,
      ...(profileSummary.foundationDraftSummaries.soul.latestMaterialSourcePath
        ? { latestMaterialSourcePath: profileSummary.foundationDraftSummaries.soul.latestMaterialSourcePath }
        : {}),
      sourceCount: profileSummary.foundationDraftSummaries.soul.sourceCount,
      materialTypes: profileSummary.foundationDraftSummaries.soul.materialTypes,
    },
    voice: {
      path: profileSummary.foundationDraftSummaries.voice.path,
      generated: true,
      generatedAt: profileSummary.foundationDraftSummaries.voice.generatedAt,
      latestMaterialAt: profileSummary.foundationDraftSummaries.voice.latestMaterialAt,
      latestMaterialId: profileSummary.foundationDraftSummaries.voice.latestMaterialId,
      ...(profileSummary.foundationDraftSummaries.voice.latestMaterialSourcePath
        ? { latestMaterialSourcePath: profileSummary.foundationDraftSummaries.voice.latestMaterialSourcePath }
        : {}),
      sourceCount: profileSummary.foundationDraftSummaries.voice.sourceCount,
      materialTypes: profileSummary.foundationDraftSummaries.voice.materialTypes,
    },
  });
  assert.deepEqual(profileSnapshot.draftSections, {
    skills: {
      generated: true,
      readySectionCount: 3,
      totalSectionCount: 3,
  ...
  assert.equal(profileSnapshot.profileSummary, 'Direct operator with a bias for momentum and fast feedback loops.');
  assert.match(profileSnapshot.snapshot, new RegExp(escapeRegExp(formatDraftSourcesLine(profileSnapshot.draftSources))));
  assert.equal(harryCommand.latestMaterialAt, profileSummary.latestMaterialAt);
  assert.equal(harryCommand.latestMaterialId, profileSummary.latestMaterialId);
  assert.equal(harryCommand.latestMaterialSourcePath, profileSummary.latestMaterialSourcePath);
  ...
  assert.equal(harryCommand.helperCommands.scaffold, harryCommand.updateIntakeCommand);
  assert.equal(harryCommand.helperCommands.updateProfile, harryCommand.updateProfileCommand);
  assert.equal(harryCommand.helperCommands.updateProfileAndRefresh, harryCommand.updateProfileAndRefreshCommand);
  assert.equal(harryCommand.helperCommands.importManifestWithoutRefresh, harryCommand.importManifestWithoutRefreshCommand);
  assert.equal(harryCommand.helperCommands.importManifest, harryCommand.importManifestCommand);
  assert.match(summary.promptPreview, new RegExp(escapeRegExp(profileSnapshot.lines[0]), 'm'));

  if (harryCommand.intakeManifestStatus === 'loaded') {
    ...
    assert.equal(harryCommand.importIntakeCommand, "node src/index.js import intake --person 'harry-han' --refresh-foundation");
    assert.equal(harryCommand.followUpImportIntakeWithoutRefreshCommand, null);
    ...
    assert.equal(summary.ingestion.recommendedInspectCommand, harryCommand.importIntakeWithoutRefreshCommand);
    ...
    assert.match(summary.promptPreview, /starter root profiles\/harry-han\/imports/);
  }

  if (profileSnapshot.refreshCommand) {
    assert.equal(summary.workLoop.currentPriority?.id, 'foundation');
    assert.equal(summary.workLoop.currentPriority?.command, profileSnapshot.refreshCommand);
    assert.equal(summary.workLoop.recommendedPriority?.command, profileSnapshot.refreshCommand);
    assert.match(summary.promptPreview, /recommended: Foundation \[queued\] — refresh Harry Han \(harry-han\)/);
  }
});
...
test('repo memory, skills, soul, and voice docs stay aligned with the structured foundation sections', () => {
  ...
  assert.match(readme, /default checked-in skill catalog stays explicit: 4 channel guides \(`channels\/feishu`, `channels\/slack`, `channels\/telegram`, `channels\/whatsapp`\), 2 profile-ingestion\/update guides \(`profiles\/intake`, `profiles\/update`\), 6 provider guides \(`providers\/anthropic`, `providers\/glm`, `providers\/kimi`, `providers\/minimax`, `providers\/openai`, `providers\/qwen`\), plus `cron` and `foundation-maintenance`/i);
  ...
  assert.match(readme, /foundation\.core\.soul\.shadowPaths.*shadowPathCount.*shadowPathSamplePaths.*shadowPathOverflowCount.*canonical `SOUL\.md` source of truth.*shadow docs like `soul\/README\.md`/i);
  ...
  assert.match(readme, /foundation\.core\.voice\.shadowPaths.*shadowPathCount.*shadowPathSamplePaths.*shadowPathOverflowCount.*canonical `voice\/README\.md` source of truth.*shadow docs like `VOICE\.md`/i);
  ...
  assert.match(readme, /foundation\.maintenance\.recommendedProfileId.*recommendedAction.*recommendedCommand.*recommendedPaths.*recommendedLatestMaterialAt.*recommendedLatestMaterialId.*recommendedLatestMaterialSourcePath.*recommendedCandidateSignalSummary.*recommendedDraftSourcesSummary.*queuedProfiles\[\*\]\.paths.*queuedProfiles\[\*\]\.latestMaterialSourcePath.*queuedProfiles\[\*\]\.draftSourcesSummary/i);
  ...
  assert.match(architectureDoc, /recommendedProfileSlices/i);
  assert.match(architectureDoc, /`starter profiles: \.\.\.`.*`recommended starter profiles: \.\.\.`.*`runnable starter profiles: \.\.\.`.*`advisory starter profiles: \.\.\.`/i);
  ...
  assert.match(architectureDoc, /foundation\.core\.soul\.shadowPaths.*shadowPathCount.*shadowPathSamplePaths.*shadowPathOverflowCount.*canonical `SOUL\.md` source of truth.*shadow docs like `soul\/README\.md`/i);
  ...
  assert.match(readme, /`draftSources` now keeps each layer's generated-or-stale draft `path` beside .*`generatedAt`, `latestMaterialAt`, `latestMaterialId`, optional `latestMaterialSourcePath`, `sourceCount`, `materialTypes`,.*`entryCount`/i);
  ...
  assert.match(architectureDoc, /canonical next-refresh target on `foundation\.maintenance`.*`recommendedProfileId`, `recommendedLabel`, `recommendedAction`, `recommendedCommand`, `recommendedPaths`, `recommendedLatestMaterialAt`, `recommendedLatestMaterialId`, `recommendedLatestMaterialSourcePath`, `recommendedCandidateSignalSummary`, `recommendedDraftSourcesSummary`, `recommendedDraftGapSummary`/i);
  ...
  assert.match(ingestionDoc, /recommendedProfileId`, `recommendedLabel`, `recommendedAction`, `recommendedCommand`, `recommendedPaths`, `recommendedLatestMaterialAt`, `recommendedLatestMaterialId`, `recommendedLatestMaterialSourcePath`, `recommendedCandidateSignalSummary`, `recommendedDraftSourcesSummary`, `recommendedDraftGapSummary`.*queuedProfiles\[0\]/i);
  assert.match(ingestionDoc, /recommendedProfileSlices/i);
  assert.match(ingestionDoc, /`starter profiles: \.\.\.`.*`recommended starter profiles: \.\.\.`.*`advisory starter profiles: \.\.\.`/i);
  ...
  assert.match(skillsDoc, /- profiles: `profiles\/intake`, `profiles\/update`/);
  ...
  assert.match(profileUpdateSkill, /`node src\/index\.js update profile --person <id> --display-name "<Display Name>" --summary "<Short summary>"`/);
  ...
  assert.match(profileUpdateSkill, /`recommendedUpdateProfileCommand`, `recommendedUpdateProfileAndRefreshCommand`, and `recommendedInspectCommand` should stay aligned with the same target profile and ready-lane `paths` bundle.*`profiles\/<id>\/profile\.json`/i);
  ...
  assert.equal(summary.foundation.core.skills.count, 14);
  assert.deepEqual(summary.skills.categoryCounts, { channels: 4, profiles: 2, providers: 6, root: 2 });
  assert.deepEqual(summary.foundation.core.skills.categoryCounts, { channels: 4, profiles: 2, providers: 6, root: 2 });
  assert.deepEqual(summary.foundation.core.skills.documentedCategoryCounts, { channels: 4, profiles: 2, providers: 6, root: 2 });
  assert.deepEqual(summary.skills.foundationStatusCounts, { ready: 14 });
