...
test('README documents the default delivery foundation targets and repo manifests', () => {
  ...
  assert.match(readme, /`recommended fallback:`, `recommended refresh intake:`, `recommended update profile:`, `recommended sync profile:`, `recommended edit:`, `recommended edit paths:`, `recommended manifest inspect:`, `recommended manifest:`, `recommended inspect after editing:`, and `recommended then run:` lines/i);
...
test('architecture and ingestion docs explain work-loop leader/blocker semantics and sample-manifest entrypoints', () => {
  ...
  assert.match(architectureDoc, /prompt-preview `Work loop:` block also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft sources`, `draft gaps`, `fallback`, `refresh intake`, `update profile`, `sync profile`, `edit` \/ `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces/i);
  ...
  assert.match(ingestionDoc, /`recommendedPriority` points at a ready edit-first ingestion slice instead of the current blocker, the same `Work loop:` block now keeps `recommended fallback`, `recommended refresh intake`, `recommended update profile`, `recommended sync profile`, `recommended edit` \/ `recommended edit paths`, `recommended manifest inspect`, `recommended manifest`, `recommended inspect after editing`, and `recommended then run` visible/i);
  ...
  assert.match(readme, /when the recommended lane is not the current blocker it also surfaces explicit `recommended fallback:`, `recommended refresh intake:`, `recommended update profile:`, `recommended sync profile:`, `recommended edit:`, `recommended edit paths:`, `recommended manifest inspect:`, `recommended manifest:`, `recommended inspect after editing:`, and `recommended then run:` lines/i);
