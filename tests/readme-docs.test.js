...
test('architecture and ingestion docs explain work-loop leader/blocker semantics and sample-manifest entrypoints', () => {
  ...
  assert.match(architectureDoc, /additive follow-up metadata \(`fallbackCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, `followUpCommand`\)/i);
  ...
  assert.match(ingestionDoc, /each `workLoop` priority row can now also carry additive follow-up metadata \(`fallbackCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, `followUpCommand`\)/i);
  ...
  assert.match(readme, /each priority can carry additive follow-up surfaces like `fallbackCommand`, `editPath`, `editPaths`, `manifestInspectCommand`, `manifestImportCommand`, `intakeManifestEntryTemplateTypes`, `intakeManifestEntryTemplateCount`, `inspectCommand`, and `followUpCommand`/i);
...
test('checked-in intake scaffold stays aligned with the repo-level starter ingress for Harry Han', () => {
  ...
  assert.match(readme, /Once an already-imported profile's drafts are fresh and it still has only the untouched `profiles\/<person-id>\/imports\/materials\.template\.json` starter manifest, the ingestion entrance keeps the top-level `next intake` step descriptive while also surfacing `recommendedEditPath`, `recommendedEditPaths`, `recommendedManifestInspectCommand`, `recommendedManifestImportCommand`, `recommendedIntakeManifestEntryTemplateTypes`, `recommendedIntakeManifestEntryTemplateCount`, `recommendedInspectCommand`, `recommendedFollowUpCommand`, and `recommendedFallbackCommand`, so operators know which file to edit in the single-profile case, which manifest set to edit in the bundled multi-profile case, which starter template entry types are still expected before import, which plain `import intake --person <id>` inspection replay to run first, which `import intake --person <id> --refresh-foundation` replay to run next, and which direct starter import still works as a fallback/i);
  ...
  assert.match(ingestionDoc, /recommendedEditPath`.*`recommendedEditPaths`.*`recommendedManifestInspectCommand`.*`recommendedManifestImportCommand`.*`recommendedIntakeManifestEntryTemplateTypes`.*`recommendedIntakeManifestEntryTemplateCount`.*`recommendedInspectCommand`.*`recommendedFollowUpCommand`/i);
