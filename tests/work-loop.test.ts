...
test('buildSummary work loop treats imported intake starter scaffolds as runnable follow-up work once direct imports already landed', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
...
test('buildSummary work loop keeps credential bootstrap paths aligned with both template source and env destination during env copy', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
  fs.writeFileSync(path.join(rootDir, '.env.example'), [
    'SLACK_BOT_TOKEN=***',
...
test('buildSummary work loop keeps blocked env bootstrap paths aligned with both template source and env destination even when checked-in delivery modules are runtime-ready', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
  fs.writeFileSync(path.join(rootDir, '.env.example'), [
    'SLACK_BOT_TOKEN=***',
