...
test('buildSummary work loop keeps credential bootstrap paths on both the template source and target during env copy', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
  fs.writeFileSync(path.join(rootDir, '.env.example'), [
    'SLACK_BOT_TOKEN=***',
...
test('buildSummary work loop keeps blocked env bootstrap paths on both the template source and target even when checked-in delivery modules are runtime-ready', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
  fs.writeFileSync(path.join(rootDir, '.env.example'), [
    'SLACK_BOT_TOKEN=***',
