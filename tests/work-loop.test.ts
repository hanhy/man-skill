...
test('buildSummary work loop keeps credential bootstrap paths focused on the template source during env copy', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
  fs.writeFileSync(path.join(rootDir, '.env.example'), [
    'SLACK_BOT_TOKEN=***',
...
test('buildSummary work loop keeps blocked env bootstrap paths focused on the template source even when checked-in delivery modules are runtime-ready', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
  fs.writeFileSync(path.join(rootDir, '.env.example'), [
    'SLACK_BOT_TOKEN=***',
