        ...
        recommendedProfileSlices: [
          {
            ...
            latestMaterialId: ' 2026-04-20T12-00-00-000Z-text ',
            latestMaterialSourcePath: ' .\\profiles\\harry-han//imports\\sample.txt ',
            refreshReasons: [' new materials ', 'new materials', ''],
            missingDrafts: [' voice ', 'voice', ''],
            fallbackCommand: null,
...
test('buildSummary work loop advances to ingestion when memory and skills root docs use alternate markdown heading styles', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
...
test('buildSummary marks ready intake manifests invalid when entry templates escape the local imports root', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
...
test('buildSummary work loop treats imported intake starter scaffolds as advisory follow-up work once direct imports already landed', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);
  ...
  assert.equal(summary.workLoop.priorities.find((priority) => priority.id === 'ingestion')?.status, 'ready');
  assert.equal(summary.workLoop.currentPriority.id, 'foundation');
...
test('buildSummary work loop keeps imported intake starter follow-up as an advisory when foundation repairs stay current', () => {
  ...
  assert.equal(summary.workLoop.currentPriority.id, 'foundation');
  assert.equal(summary.workLoop.currentPriority.status, 'queued');
  assert.equal(summary.workLoop.runnablePriority?.id, 'foundation');
  ...
  assert.equal(summary.workLoop.actionableReadyPriority?.manifestInspectCommand, "node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json'");
  ...
  assert.match(summary.promptPreview, /advisory command: node src\/index\.js import manifest --file 'profiles\/harry-han\/imports\/materials\.template\.json'/);
...
test('buildSummary work loop carries imported starter intake edit and follow-up guidance when ingestion stays current', () => {
  ...
  assert.equal(summary.workLoop.currentPriority.command, "node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json'");
  ...
  assert.match(
    summary.workLoop.currentPriority?.updateProfileAndRefreshCommand ?? '',
  ...
  assert.match(summary.promptPreview, /evidence: memory 1 \(text\) \| voice 1 \(text\) \| soul 1 \(text\) \| skills 0/);
  assert.match(summary.promptPreview, /draft sources: memory 1 source \(text:1\), 1 entry, latest @ samples\/harry-post\.txt \| skills @ profiles\/harry-han\/skills\/README\.md \| soul 1 source \(text:1\), latest @ samples\/harry-post\.txt \| voice 1 source \(text:1\), latest @ samples\/harry-post\.txt/);
...
test('buildSummary work loop surfaces imported starter-manifest edits as advisory follow-up work when delivery auth bootstrap is current', () => {
  ...
  assert.match(
    summary.workLoop.actionableReadyPriority?.updateProfileCommand ?? '',
    /^node src\/index\.js update profile --person 'harry-han' --display-name 'Harry Han'(?: --summary '.*')?$/,
  );
  assert.match(
    summary.workLoop.actionableReadyPriority?.updateProfileAndRefreshCommand ?? '',
    /^node src\/index\.js update profile --person 'harry-han' --display-name 'Harry Han'(?: --summary '.*')? --refresh-foundation$/,
  );
  ...
  assert.match(summary.promptPreview, /runnable next action: populate the imported intake starter manifest for Harry Han \(harry-han\)/);
  assert.match(summary.promptPreview, /runnable command: node src\/index\.js import manifest --file 'profiles\/harry-han\/imports\/materials\.template\.json'/);
  assert.ok(summary.promptPreview.includes(`runnable latest material: ${advisoryLatestMaterial}`));
  ...
  assert.match(summary.promptPreview, /runnable starter templates: message, screenshot, talk, text \(4 total\)/);
...
test('buildSummary work loop keeps the imported intake replay bundle advisory when multiple starter manifests need edits', () => {
  ...
  assert.equal(summary.workLoop.actionableReadyPriority?.manifestInspectCommand, "(node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json') && (node src/index.js import manifest --file 'profiles/jane-doe/imports/materials.template.json')");
  assert.equal(summary.workLoop.actionableReadyPriority?.manifestImportCommand, "(node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json' --refresh-foundation) && (node src/index.js import manifest --file 'profiles/jane-doe/imports/materials.template.json' --refresh-foundation)");
  assert.equal(summary.workLoop.actionableReadyPriority?.editPath, 'profiles/harry-han/imports/materials.template.json');
  ...
  assert.equal(summary.workLoop.recommendedPriority?.manifestInspectCommand, "(node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json') && (node src/index.js import manifest --file 'profiles/jane-doe/imports/materials.template.json')");
  assert.equal(summary.workLoop.recommendedPriority?.manifestImportCommand, "(node src/index.js import manifest --file 'profiles/harry-han/imports/materials.template.json' --refresh-foundation) && (node src/index.js import manifest --file 'profiles/jane-doe/imports/materials.template.json' --refresh-foundation)");
  assert.equal(summary.workLoop.recommendedPriority?.editPath, 'profiles/harry-han/imports/materials.template.json');
  ...
  assert.match(summary.promptPreview, /runnable manifest inspect: \(node src\/index\.js import manifest --file 'profiles\/harry-han\/imports\/materials\.template\.json'\) && \(node src\/index\.js import manifest --file 'profiles\/jane-doe\/imports\/materials\.template\.json'\)/);
  assert.match(summary.promptPreview, /runnable manifest: \(node src\/index\.js import manifest --file 'profiles\/harry-han\/imports\/materials\.template\.json' --refresh-foundation\) && \(node src\/index\.js import manifest --file 'profiles\/jane-doe\/imports\/materials\.template\.json' --refresh-foundation\)/);
  assert.match(summary.promptPreview, /runnable starter templates: message, screenshot, talk, text \(4 total\)/);
  assert.match(summary.promptPreview, /runnable starter details: message Keep the operating note concise\. \| screenshot images\/chat\.png \| talk Ship the correction while it is still fresh\. \| text sample\.txt/);
  assert.match(summary.promptPreview, /runnable edit paths: profiles\/harry-han\/imports\/materials\.template\.json, profiles\/harry-han\/imports\/sample\.txt, profiles\/jane-doe\/imports\/materials\.template\.json, profiles\/jane-doe\/imports\/images\/chat\.png, profiles\/jane-doe\/imports\/sample\.txt/);
...
test('buildSummary work loop slash-normalizes ready intake manifest file assets before collecting manifest-backed paths', () => {
  ...
  const intakeDir = path.join(rootDir, 'profiles', 'metadata-only', 'imports');
  fs.mkdirSync(path.join(intakeDir, 'images'), { recursive: true });
  fs.writeFileSync(path.join(intakeDir, 'images', 'metadata-shot.png'), 'fake screenshot bytes\n');
  fs.writeFileSync(
    path.join(intakeDir, 'materials.template.json'),
    JSON.stringify({
      personId: 'Metadata Only',
      entries: [
        { type: 'text', file: '.\\sample.txt' },
        { type: 'screenshot', file: '.\\images\\metadata-shot.png' },
      ],
    }, null, 2),
  );
...
test('buildSummary work loop stays on the leading ready priority once every priority is ready', () => {
  ...
  assert.match(workLoopBlock, /- runnable refresh intake: node src\/index\.js update intake --person 'harry-han' --display-name 'Harry Han'(?: --summary '.*')?/);
  assert.match(workLoopBlock, /- runnable edit: profiles\/harry-han\/imports\/materials\.template\.json/);
  assert.match(workLoopBlock, /- runnable edit paths: profiles\/harry-han\/imports\/materials\.template\.json, profiles\/harry-han\/imports\/images\/chat\.png, profiles\/harry-han\/imports\/sample\.txt/);
...
test('buildSummary work loop skips env bootstrap once a repo-local .env already exists', () => {
  ...
  assert.equal(summary.delivery.channelQueue[0].helperCommands.bootstrapEnv, null);
  assert.equal(summary.delivery.providerQueue[0].helperCommands.bootstrapEnv, null);
  assert.equal(summary.workLoop.currentPriority.nextAction, 'create manifests/channels.json; set FEISHU_APP_ID, FEISHU_APP_SECRET');
  ...
  assert.doesNotMatch(summary.promptPreview, /env bootstrap: cp \.env\.example \.env/);
  assert.doesNotMatch(summary.promptPreview, /\| helpers: env cp \.env\.example \.env/);
  assert.match(summary.promptPreview, /next action: create manifests\/channels\.json; set FEISHU_APP_ID, FEISHU_APP_SECRET/);
...
test('buildSummary work loop scaffolds the channel manifest once the queue leader is already configured', () => {
  ...
  try {
    ...
    assert.equal(summary.workLoop.currentPriority.id, 'channels');
    assert.equal(summary.workLoop.currentPriority.nextAction, 'create manifests/channels.json; credentials present');
    assert.equal(summary.workLoop.currentPriority.command, "mkdir -p 'manifests' && touch 'manifests/channels.json'");
    assert.deepEqual(summary.workLoop.currentPriority.paths, ['manifests/channels.json', 'src/channels/feishu.js']);
    assert.match(summary.promptPreview, /next action: create manifests\/channels\.json; credentials present/);
    assert.match(summary.promptPreview, /command: mkdir -p 'manifests' && touch 'manifests\/channels\.json'/);
    assert.doesNotMatch(summary.promptPreview, /paths: .*\.env\.example/);
  } finally {
...
test('buildSummary work loop bundles missing channel implementations once the channel manifest exists and credentials are configured', () => {
  ...
  try {
    ...
    assert.equal(summary.workLoop.currentPriority.id, 'channels');
    assert.equal(summary.workLoop.currentPriority.nextAction, 'create pending channel implementations — starting with src/channels/feishu.js; credentials present');
    ...
    assert.match(summary.promptPreview, /next action: create pending channel implementations — starting with src\/channels\/feishu\.js; credentials present/);
...
test('buildSummary work loop keeps scaffold-only delivery files visible as runtime backlog', () => {
  ...
  assert.equal(summary.workLoop.currentPriority.nextAction, 'implement pending channel integrations — starting with src/channels/feishu.js; set FEISHU_APP_ID, FEISHU_APP_SECRET');
  assert.equal(summary.workLoop.currentPriority.command, null);
  ...
  assert.match(summary.promptPreview, /next action: implement pending channel integrations — starting with src\/channels\/feishu\.js; set FEISHU_APP_ID, FEISHU_APP_SECRET/);
...
test('buildSummary work loop still scaffolds missing delivery files before scaffold-only ones', () => {
  ...
  assert.equal(summary.workLoop.currentPriority.nextAction, 'create pending channel implementations — starting with src/channels/feishu.js; set FEISHU_APP_ID, FEISHU_APP_SECRET');
...
test('buildSummary work loop paths follow the actual missing delivery file when a scaffold-only leader stays ahead of a single missing implementation', () => {
  ...
  assert.equal(summary.workLoop.currentPriority.nextAction, 'create src/channels/feishu.js; set FEISHU_APP_ID, FEISHU_APP_SECRET');
  assert.deepEqual(summary.workLoop.currentPriority.paths, ['manifests/channels.json', 'src/channels/feishu.js']);
...
test('buildSummary work loop bundles missing provider implementations once the provider manifest exists', () => {
  ...
  assert.equal(summary.workLoop.currentPriority.id, 'providers');
  assert.equal(summary.workLoop.currentPriority.nextAction, 'create pending provider implementations — starting with src/models/openai.js; auth configured for gpt-5');
  ...
  assert.match(summary.promptPreview, /next action: create pending provider implementations — starting with src\/models\/openai\.js; auth configured for gpt-5/);
...
test('buildSummary keeps ingestion refresh ordering aligned with foundation maintenance when stale imported profiles only differ by latest material source path', () => {
  const rootDir = makeTempRepo();
  seedReadyFoundationRepo(rootDir);

  fs.mkdirSync(path.join(rootDir, 'samples'), { recursive: true });
  fs.writeFileSync(path.join(rootDir, 'samples', 'alpha-source.txt'), 'Alpha operator note.\n');
  fs.writeFileSync(path.join(rootDir, 'samples', 'zeta-source.txt'), 'Beta operator note.\n');

  runImportCommand(rootDir, 'text', {
    person: 'alpha-operator',
    file: path.join(rootDir, 'samples', 'alpha-source.txt'),
    'refresh-foundation': true,
  });
  runImportCommand(rootDir, 'text', {
    person: 'beta-operator',
    file: path.join(rootDir, 'samples', 'zeta-source.txt'),
    'refresh-foundation': true,
  });

  const rewriteLatestMaterial = (profileId: string) => {
    const materialsDir = path.join(rootDir, 'profiles', profileId, 'materials');
    const [materialFile] = fs.readdirSync(materialsDir).filter((file) => file.endsWith('.json'));
    assert.ok(materialFile);
    const materialPath = path.join(materialsDir, materialFile);
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...record,
        id: '2026-04-20T12-00-00-000Z-shared',
        createdAt: '2026-04-20T12:00:00.000Z',
      }, null, 2),
    );
  };

  rewriteLatestMaterial('alpha-operator');
  rewriteLatestMaterial('beta-operator');
  ...
  assert.equal(summary.workLoop.currentPriority.command, summary.foundation.maintenance.refreshBundleCommand);
  assert.equal(summary.workLoop.priorities[1].command, summary.ingestion.refreshFoundationBundleCommand);
  assert.match(summary.promptPreview, /latest material 2026-04-20T12:00:00\.000Z \(2026-04-20T12-00-00-000Z-shared\) @ samples\/zeta-source\.txt/);
  assert.match(summary.promptPreview, /next intake: refresh stale or incomplete target profiles; command \(node src\/index\.js update foundation --person 'beta-operator'\) && \(node src\/index\.js update foundation --person 'alpha-operator'\)/);
});
