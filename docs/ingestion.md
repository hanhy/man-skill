...
- top-level `delivery` setup guidance that turns channel/provider metadata into an operator queue (`pendingChannelCount`, `pendingProviderCount`, `channelManifestPath`, `providerManifestPath`, `channelQueue`, `providerQueue`)
  - each queued channel/provider row now includes a concrete `setupHint` (for example `set SLACK_BOT_TOKEN, SLACK_SIGNING_SECRET` or `set OPENAI_API_KEY for gpt-5`) so the prompt preview can show rollout work as the next action instead of only listing adapter names
