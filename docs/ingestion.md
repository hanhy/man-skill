...
- the prompt-preview `Work loop:` block now also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft sources`, `draft gaps`, `fallback`, `refresh intake`, `update profile`, `sync profile`, `edit` / `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces instead of hiding that post-edit contract inside the ingestion summary alone
  ...
  - when an already-imported profile's drafts are already fresh and it still only has the untouched `profiles/<id>/imports/materials.template.json` starter scaffold, the same top-level recommendation keeps `recommendedCommand` empty but now exposes `recommendedLatestMaterialAt`, `recommendedLatestMaterialId`, `recommendedLatestMaterialSourcePath`, `recommendedRefreshIntakeCommand`, `recommendedUpdateProfileCommand`, `recommendedUpdateProfileAndRefreshCommand`, `recommendedEditPath`, `recommendedEditPaths`, `recommendedManifestInspectCommand`, `recommendedManifestImportCommand`, `recommendedIntakeManifestEntryTemplateTypes`, `recommendedIntakeManifestEntryTemplateDetails`, `recommendedIntakeManifestEntryTemplateCount`, `recommendedIntakeManifestEntryTemplateRoot`, `recommendedProfileSlices`, `recommendedInspectCommand`, and `recommendedFollowUpCommand` so `next intake` can say which newest imported artifact triggered the follow-up, which intake scaffold rerun restores the profile-local landing zone before import, which plain metadata edit and metadata-sync-with-refresh commands remain available beside that starter-manifest work, which manifest to edit first in the single-profile case, which broader starter-edit surface to touch in bundled or file-backed cases (`materials.template.json` plus seeded files like `sample.txt` / `images/chat.png`), which shared `profiles/<id>/imports` starter root those relative file-backed template paths resolve under, which starter template entry types and concrete placeholder details are still expected before import, which per-profile `label -> edit path` slices belong to each queued imported starter manifest when several are bundled together, which per-slice `refreshReasons`, `missingDrafts`, and `draftGapSummary` keep those bundled edit rows aligned with the same profile-foundation state, which direct `import manifest --file 'profiles/<id>/imports/materials.template.json'` inspect/import commands remain available during that edit, which plain `import intake --person <id>` inspection replay should run right after the manifest edit, and which `import intake --person <id> --refresh-foundation` replay should run after that when the same pass should regenerate memory / voice / soul / skills drafts too
