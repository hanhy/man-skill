...
- the prompt-preview `Work loop:` block now also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft sources`, `draft gaps`, `fallback`, `refresh intake`, `update profile`, `sync profile`, `edit` / `edit paths`, `manifest inspect`, `manifest`, `inspect first|after editing`, and `then run` surfaces instead of hiding that follow-up contract inside the ingestion summary alone
  ...
  - once that same already-imported profile has a runnable profile-local manifest, the recommendation can promote `recommendedCommand` to `node src/index.js import intake --person '<id>' --refresh-foundation` while still preserving `recommendedInspectCommand`, `recommendedUpdateProfileCommand`, `recommendedUpdateProfileAndRefreshCommand`, and the same `recommendedPaths` landing-zone bundle (`materials.template.json`, `sample.txt`, `images/chat.png`), so the ready replay never hides the plain inspection pass or the adjacent metadata-edit helpers
