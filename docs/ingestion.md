...
- top-level `ingestion` entrance data (`profileCount`, `importedProfileCount`, `metadataOnlyProfileCount`, `readyProfileCount`, `refreshProfileCount`, `incompleteProfileCount`, `importedIntakeReadyProfileCount`, `importedIntakeBackfillProfileCount`, `supportedImportTypes`, `bootstrapProfileCommand`, `sampleImportCommand`, `importManifestCommand`, `sampleManifestPath`, `sampleManifestPresent`, `sampleStarterCommand`, `sampleStarterSource`, `sampleManifestCommand`, `sampleTextPath`, `sampleTextPresent`, `sampleTextCommand`, `staleRefreshCommand`, `intakeImportedCommand`, `intakeImportImportedCommand`, `intakeImportImportedAndRefreshCommand`, `updateProfileBundleCommand`, `updateProfileAndRefreshBundleCommand`, `recommendedProfileId`, `recommendedLabel`, `recommendedAction`, `recommendedCommand`, `recommendedFallbackCommand`, `recommendedEditPath`, `recommendedEditPaths`, `recommendedManifestInspectCommand`, `recommendedManifestImportCommand`, `recommendedIntakeManifestEntryTemplateTypes`, `recommendedIntakeManifestEntryTemplateCount`, `recommendedInspectCommand`, `recommendedFollowUpCommand`, `recommendedPaths`, `helperCommands`, `profileCommands`, `allProfileCommands`) so the summary exposes the default material-import/update commands alongside the foundation maintenance queue
  ...
  - when a priority is advisory rather than immediately runnable — for example imported starter-manifest edits — the prompt-preview `Work loop:` block now also serializes the same `fallback`, `edit` / `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces instead of hiding that post-edit contract inside the ingestion summary alone
  ...
  - when an already-imported profile's drafts are already fresh and it still only has the untouched `profiles/<id>/imports/materials.template.json` starter scaffold, the same top-level recommendation keeps `recommendedCommand` empty but now exposes `recommendedEditPath`, `recommendedEditPaths`, `recommendedManifestInspectCommand`, `recommendedManifestImportCommand`, `recommendedIntakeManifestEntryTemplateTypes`, `recommendedIntakeManifestEntryTemplateCount`, `recommendedInspectCommand`, and `recommendedFollowUpCommand` so `next intake` can say which manifest to edit in the single-profile case, which manifest set to edit in the bundled multi-profile case, which starter template entry types are still expected before import, which direct `import manifest --file 'profiles/<id>/imports/materials.template.json'` inspect/import commands remain available during that edit, which plain `import intake --person <id>` inspection replay comes immediately after that edit, and which `import intake --person <id> --refresh-foundation` replay comes after that
