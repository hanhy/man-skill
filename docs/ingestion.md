...
- top-level `ingestion` entrance data (`profileCount`, `importedProfileCount`, `metadataOnlyProfileCount`, `readyProfileCount`, `refreshProfileCount`, `incompleteProfileCount`, `importedIntakeReadyProfileCount`, `importedIntakeBackfillProfileCount`, `supportedImportTypes`, `bootstrapProfileCommand`, `sampleImportCommand`, `importManifestCommand`, `sampleManifestPath`, `sampleManifestPresent`, `sampleStarterCommand`, `sampleStarterSource`, `sampleManifestCommand`, `sampleTextPath`, `sampleTextPresent`, `sampleTextCommand`, `staleRefreshCommand`, `intakeImportedCommand`, `intakeImportImportedCommand`, `intakeImportImportedAndRefreshCommand`, `updateProfileBundleCommand`, `updateProfileAndRefreshBundleCommand`, `recommendedProfileId`, `recommendedLabel`, `recommendedAction`, `recommendedCommand`, `recommendedFallbackCommand`, `recommendedEditPath`, `recommendedEditPaths`, `recommendedManifestInspectCommand`, `recommendedManifestImportCommand`, `recommendedInspectCommand`, `recommendedFollowUpCommand`, `recommendedPaths`, `helperCommands`, `profileCommands`, `allProfileCommands`) so the summary exposes the default material-import/update commands alongside the foundation maintenance queue
  - the top-level `workLoop` summary now also exposes both `leadingPriority` and `currentPriority`, plus `runnablePriority` for the first still-runnable step in priority order even when that work remains a ready follow-up such as imported starter-manifest edits, `actionableReadyPriority` as the ready-only advisory alias, and `recommendedPriority` as the stable best-next-action alias that prefers runnable work before falling back to the current blocker or the already-ready leader, alongside the split readiness counters (`readyPriorityCount`, `queuedPriorityCount`, `blockedPriorityCount`); the same `USER.md` current product direction loader ignores fenced or commented scaffold headings so only visible objectives drive the work loop, while still accepting blockquoted visible headings and list items, and downstream tooling can distinguish “first in order”, queued-or-blocked gating work, runnable ready follow-ups, auth-blocked delivery rollout, and the single recommended next slice without re-deriving state from prompt prose alone
