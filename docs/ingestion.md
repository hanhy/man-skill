...
- `buildSummary(...).profileSnapshots[]` mirrors the compact operator-facing profile foundation snapshot surface as machine-readable records (`id`, `label`, `snapshot`, `lines`) so downstream tooling can consume the same per-profile draft summary without scraping `promptPreview`
