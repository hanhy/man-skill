...
- top-level `ingestion` entrance data (`profileCount`, `importedProfileCount`, `metadataOnlyProfileCount`, `readyProfileCount`, `refreshProfileCount`, `incompleteProfileCount`, `importManifestCommand`, `staleRefreshCommand`, `profileCommands`) so the summary exposes the default material-import/update commands alongside the foundation maintenance queue
  - the prompt preview mirrors this as `Ingestion entrance:` before delivery/foundation diagnostics so operators can jump straight from the summary to `import manifest`, `update profile`, or `update foundation`
