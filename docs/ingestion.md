...
- top-level `foundation.core` repo diagnostics for the base memory / skills / soul / voice scaffold, including an `overview` block (`readyAreaCount`, `missingAreas`, `thinAreas`) that the prompt preview mirrors as a compact coverage line
  ...
  - `foundation.core.maintenance` also carries per-area runnable commands plus a grouped helper bundle (`helperCommands.scaffoldAll`, `memory`, `skills`, `soul`, `voice`), so the repo-level memory / skills / soul / voice backlog is directly executable from JSON and the prompt preview instead of only described in prose
