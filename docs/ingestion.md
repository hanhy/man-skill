...
- the prompt-preview `Work loop:` block now also serializes the same `latest material`, `refresh reasons`, `missing drafts`, `evidence`, `draft sources`, `draft gaps`, `fallback`, `refresh intake`, `update profile`, `sync profile`, `edit` / `edit paths`, `manifest inspect`, `manifest`, `inspect after editing`, and `then run` surfaces instead of hiding that post-edit contract inside the ingestion summary alone
  ...
  - the prompt-preview helper palette mirrors those machine-readable helpers with stable labels like `repair-invalid-bundle`, `repair-imported-invalid-bundle`, `inspect-starter-bundle`, `replay-starter-bundle`, `starter-import-bundle`, `update-bundle`, `sync-bundle`, `sample-message`, `sample-talk`, and `sample-screenshot`, so invalid intake repairs, imported starter-manifest replays, metadata sync, and non-text starter imports stay copy-pasteable in the same user-facing entrance
