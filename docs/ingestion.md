...
- top-level `foundation.core` repo diagnostics for the base memory / skills / soul / voice scaffold, including an `overview` block (`readyAreaCount`, `missingAreas`, `thinAreas`) that the prompt preview mirrors as a compact coverage line
  ...
  - that repo-core memory surface also carries `foundation.core.memory.legacyShortTermSourceCount` and `foundation.core.memory.legacyShortTermSources`, so ingestion-side tooling can preserve exactly which checked-in `memory/short-term/` files were folded into `daily/`
  ...
  - the top-level memory summary also mirrors the legacy-source provenance via `memorySummary.legacyShortTermSourceCount` and `memorySummary.legacyShortTermSources`, so work-loop consumers can inspect the original `memory/short-term/` paths without reloading the repo tree
