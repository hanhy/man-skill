...
- mirror those same compact per-profile draft snapshots into top-level `buildSummary(...).profileSnapshots[]` records (`id`, `label`, `snapshot`, `lines`) so downstream tooling can consume the same operator-facing foundation view without re-parsing `promptPreview`
