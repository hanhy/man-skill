...
- make delivery work-loop routing leader-aware: only surface `cp .env.example .env` when the current queued channel/provider is truly blocked on missing env vars and no repo-local `.env` exists yet; once `.env` is already present, switch the blocked rollout step to the narrower `touch '.env' && ...` populate helper so `paths` narrows to `.env`, otherwise promote manifest/implementation scaffolds and keep `paths` aligned with the actual command blast radius; during bootstrap that means `paths` includes both `.env.example` and `.env` so the copy step's read/write blast radius stays explicit
