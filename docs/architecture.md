...
- expose a repo-core `foundation.core.maintenance` queue with per-area (`memory`, `skills`, `soul`, `voice`) `missing` vs `thin` status, expected file paths, the next concrete action, stored runnable commands, thin-doc section progress (`rootThinReadySections`, `rootThinMissingSections`, `rootThinReadySectionCount`, `rootThinTotalSectionCount`), a canonical single-area recommendation (`recommendedArea`, `recommendedStatus`, `recommendedSummary`, `recommendedAction`, `recommendedCommand`, `recommendedPaths`), and helper bundles (`scaffoldAll`, `scaffoldMissing`, `scaffoldThin`, `memory`, `skills`, `soul`, `voice`) so operators can fix scaffold drift without inferring it from aggregate counts alone
