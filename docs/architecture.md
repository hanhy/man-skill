...
- summarize target-person ingestion status through a top-level `ingestion` block with imported-vs-metadata-only counts, default CLI entry commands, exact helper-command bundles for scaffold/import/refresh work, matching top-level bundle mirrors for metadata updates (`updateProfileBundleCommand`, `updateProfileAndRefreshBundleCommand`), both plain and `--refresh-foundation` manifest import shortcuts, a generic inspect-first manifest helper via `importManifestInspectCommand`, the exact checked-in sample manifest inspect command via `sampleManifestInspectCommand`, the exact checked-in sample manifest refresh command via `sampleManifestCommand`, that sample manifest's typed material mix, a shorter `import sample` starter command that wraps that manifest when it validates, the exact starter-manifest source path surfaced via `sampleStarterSource`, a checked-in sample text command, the keyed sample helper labels (`sample-message`, `sample-talk`, `sample-screenshot`), imported-starter-manifest helper labels (`inspect-starter-bundle`, `replay-starter-bundle`, `starter-import-bundle`), explicit invalid-intake repair plus inspect/replay bundles (`repair-invalid-bundle`, `inspect-invalid-bundle`, `replay-invalid-bundle`, `repair-imported-invalid-bundle`, `inspect-imported-invalid-bundle`, `replay-imported-invalid-bundle`), the bundled per-profile starter follow-up slices via `recommendedProfileSlices`, the top-level starter-follow-up metadata-edit helpers (`recommendedUpdateProfileCommand`, `recommendedUpdateProfileAndRefreshCommand`), and the first actionable per-profile `update profile` / `update foundation` commands plus batch metadata helpers (`update-bundle`, `sync-bundle`)
- `recommendedProfileSlices` also mirror each queued starter profile's `refreshReasons`, `missingDrafts`, and `draftGapSummary`, so bundled edit-first follow-ups keep the same draft-health context as the main per-profile command rows
...
- carry per-draft provenance forward (`latestMaterialId`, `latestMaterialSourcePath`, `materialTypes`, markdown draft headers including `Generated at`, `Latest material`, `Latest material source`, and `Source materials`) so generated memory / voice / soul / skills artifacts remain auditable after ingestion; the checked-in sample profile drafts under `profiles/harry-han/voice/README.md`, `profiles/harry-han/soul/README.md`, and `profiles/harry-han/skills/README.md` intentionally keep that same header contract visible in the repo fixture itself
- mirror memory-draft provenance (`generatedAt`, latest material id/timestamp, `latestMaterialSourcePath`, source counts, material types) back into `foundationDraftSummaries.memory` so prompt/runtime consumers can inspect freshness without reopening raw draft files; the persisted `sourceCount` is part of stale-refresh evidence too, so `update foundation --stale` can repair memory drafts whose source coverage drifted even when their entry counts did not
...
- also surface `foundation.core.soul.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` so the canonical `SOUL.md` source of truth stays explicit when shadow docs like `soul/README.md` exist beside it, and JSON callers can reuse the same first-three-plus-overflow duplicate-doc preview as the prompt layer
- do the same for repo-core prose layers by surfacing `foundation.core.voice.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` for the canonical `voice/README.md` source of truth when shadow docs like `VOICE.md` exist, so duplicate-root previews stay aligned for voice too
- likewise surface `foundation.core.memory.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` for the canonical `memory/README.md` source of truth when shadow docs like `MEMORY.md` exist, and `foundation.core.skills.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` for the canonical `skills/README.md` source of truth when shadow docs like `SKILLS.md` exist, so duplicate-root previews stay aligned across memory and skills as well
...
- when the work loop surfaces one of those thin repo-core repairs as `currentPriority` or `recommendedPriority`, also mirror `workLoop.currentPriority.shadowPathCount`, `workLoop.currentPriority.shadowPathSamplePaths`, and `workLoop.currentPriority.shadowPathOverflowCount` (with the same fields reused on `workLoop.recommendedPriority` when that repair is the recommendation) beside any full `shadowPaths` list so downstream tooling can reuse the same first-three-plus-overflow duplicate-doc preview contract without reparsing prompt text or requiring the entire shadow-doc array to survive
...
- reuse shared `normalizeDraftPath(...)` path cleanup across draft-path and source-provenance surfaces so `draftFiles`, `draftSources`, `queuedProfiles[*].latestMaterialSourcePath`, and prompt-side `latest material` / `draft sources` lines stay slash-normalized even when direct callers provide `./`-prefixed or backslash-heavy paths
- `draftSources` preserve each layer's draft `path` alongside provenance metadata so downstream tooling can inspect the same generated or stale artifact without cross-referencing `draftFiles`, and the prompt-side `draft sources:` snapshot line keeps that path visible even when count-based provenance is present, can append `latest @ <source-path>` when counts stay available, and still falls back to compact `memory @ profiles/...` path summaries when stale drafts no longer carry source counts yet; when the latest draft-driving material came from a concrete file, the same structured snapshot data keeps `latestMaterialSourcePath` beside the draft artifact path so downstream consumers do not need to scrape the human string to recover the freshest backing file
- human `draft files:` snapshot line mirrors `draftFiles` in canonical memory → skills → soul → voice order so operators can open the generated or stale artifacts before inspecting highlights or draft gaps
...
- expose a canonical next-refresh target on `foundation.maintenance` (`recommendedProfileId`, `recommendedLabel`, `recommendedAction`, `recommendedCommand`, `recommendedPaths`, `recommendedLatestMaterialAt`, `recommendedLatestMaterialId`, `recommendedLatestMaterialSourcePath`, `recommendedCandidateSignalSummary`, `recommendedDraftSourcesSummary`, `recommendedDraftGapSummary`) so prompt assembly and the work loop can reuse one stable per-profile refresh recommendation instead of recomputing it from the queue order
...
- expose starter-manifest intake reruns through top-level `recommendedRefreshIntakeCommand` and work-loop `refreshIntakeCommand`, plus matching metadata edit/sync follow-ups through `recommendedUpdateProfileCommand` / `recommendedUpdateProfileAndRefreshCommand` and work-loop `updateProfileCommand` / `updateProfileAndRefreshCommand`, so edit-first imported intake follow-ups can restore the profile-local landing zone and keep target-profile metadata maintenance visible both in the top-level recommendation and in each starter-template profile row instead of collapsing into lower-level manifest replay only
...
- prompt preview also keeps compact delivery readiness lines like `runtime implementations: ...`, `auth readiness: ...`, `channel env backlog: ...`, and `provider env backlog: ...` so operators can see what is blocked before opening the raw delivery queue
- helper labels `env`, `delivery env`, `channel env`, and `provider env` stay stable between JSON helper bundles and the rendered prompt palette
