...
- also expose `foundation.core.memory.rootReadySections` / `rootMissingSections` so the memory line can show `root sections X/2 ready (...)` alongside the bucket-level `buckets X/3 ready (...)` coverage state
...
- for structured soul/voice docs, also expose `readySections`, `missingSections`, `readySectionCount`, and `totalSectionCount`, so prompt previews and maintenance summaries can show grounded section labels for both partial and fully ready identity docs
