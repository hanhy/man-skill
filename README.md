...
- the default checked-in skill catalog stays explicit: 4 channel guides (`channels/feishu`, `channels/slack`, `channels/telegram`, `channels/whatsapp`), 1 profile-ingestion guide (`profiles/intake`), 6 provider guides (`providers/anthropic`, `providers/glm`, `providers/kimi`, `providers/minimax`, `providers/openai`, `providers/qwen`), plus `cron` and `foundation-maintenance`
