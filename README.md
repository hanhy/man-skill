...
This creates a profile-specific material structure under `profiles/<person-id>/` and exposes a repo summary that can later feed the learning/update layer. You can seed target-person metadata with `update profile`, via a manifest `profiles` block, or via the single-target manifest shorthand, then regenerate derived memory / voice / soul / skills drafts with `--refresh-foundation` or `update foundation --stale`. The repo summary now exposes per-profile draft freshness plus a repo-level `foundation` rollup, a compact `foundation.maintenance` queue for profiles that still need refreshes, and `foundation.core` diagnostics that distinguish documented skills (`skills/<name>/SKILL.md`) from placeholder skill folders so thin skill scaffolding shows up immediately in the prompt preview instead of being counted as ready coverage. Core memory diagnostics now also track bucket coverage across `memory/daily`, `memory/long-term`, and `memory/scratch`, so partial scaffolds stay visibly thin until all three lanes are seeded. The maintenance queue now also surfaces ready-to-run refresh commands (`node src/index.js update foundation --stale` and per-profile `--person <id>` commands) so operators can jump directly from summary to action. A separate top-level `ingestion` summary now keeps the user-facing entrance explicit with imported-vs-metadata-only counts, supported import types (`message`, `screenshot`, `talk`, `text`), a bootstrap `update profile` command, a sample one-shot text import command with `--refresh-foundation`, the default `import manifest` / `update foundation --stale` shortcuts, and the first few per-profile `update profile` / `update foundation` commands that the prompt preview mirrors under `Ingestion entrance:` even before any target profile has been imported. A top-level `delivery` summary now also keeps channel/provider rollout work operational by surfacing pending setup queues, manifest paths, auth-readiness counts, and concrete env-var hints for the default Slack / Telegram / WhatsApp / Feishu and OpenAI / Anthropic / Kimi / Minimax / GLM / Qwen integrations, which the prompt preview mirrors under `Delivery foundation:`. When the repo scaffold is missing or thin, `foundation.core.overview.recommendedActions` also suggests the next concrete fixes (for example creating `voice/README.md`, documenting placeholder skill folders, or filling empty memory buckets). Delivery summaries now also aggregate channel/provider status counts (`active`, `planned`, `candidate`) plus deduped auth env-var requirements, so the same summary can double as a lightweight rollout checklist for Slack / Telegram / WhatsApp / Feishu and OpenAI / Anthropic / Kimi / Minimax / GLM / Qwen instead of only a label dump.
