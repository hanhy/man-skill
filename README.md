...
Re-running `update intake` preserves starter `entries[]`, customized `entryTemplates`, and the README's managed `Custom notes` block while still syncing generated commands plus top-level display name and summary metadata. If `materials.template.json` was edited into invalid JSON, the repair pass now snapshots the broken file to `materials.template.json.invalid-<timestamp>.bak` and returns `invalidStarterManifestBackupPath` in the CLI result before rebuilding the starter manifest, so operators can recover the last draft instead of losing it during intake repair. The repo also ships a starter manifest at `samples/harry-materials.json` plus checked-in sample assets, so there is a one-command bootstrap path via `node src/index.js import sample`, a plain inspect-first path via `sampleManifestInspectCommand`, and an exact refresh path via `sampleManifestCommand` alongside the explicit sample helper commands in the summary/prompt preview. Once an already-imported profile's drafts are fresh and it still has only the untouched `profiles/<person-id>/imports/materials.template.json` starter manifest, the ingestion entrance keeps the top-level `next intake` step descriptive while also surfacing `recommendedRefreshIntakeCommand`, `recommendedEditPath`, `recommendedEditPaths`, `recommendedManifestInspectCommand`, `recommendedManifestImportCommand`, `recommendedIntakeManifestEntryTemplateTypes`, `recommendedIntakeManifestEntryTemplateCount`, `recommendedInspectCommand`, `recommendedFollowUpCommand`, and `recommendedFallbackCommand`, so operators know which intake scaffold rerun restores the profile-local landing zone before import, which file to edit in the single-profile case, which manifest set to edit in the bundled multi-profile case, which starter template entry types are still expected before import, which plain `import intake --person <id>` inspection replay to run first, which `import intake --person <id> --refresh-foundation` replay to run next, and which direct starter import still works as a fallback before the manifest is customized. The per-profile prompt-preview line now also keeps the local scaffold refresh visible as `refresh-intake node src/index.js update intake --person ...`, an `inspect-after-edit node src/index.js import intake --person ...` replay for a plain post-edit inspection pass, and a matching `replay-after-edit node src/index.js import intake --person ... --refresh-foundation` follow-up. Batch `update intake --stale` and `update intake --all` remain scaffold-only metadata prep, so even when `--refresh-foundation` is present they do not import materials or regenerate drafts on their own.
...
- `foundation.maintenance.recommendedProfileId`, `recommendedAction`, `recommendedCommand`, and `recommendedPaths` point at the next target-profile draft refresh, while `recommendedLatestMaterialAt` / `recommendedLatestMaterialId` / `recommendedLatestMaterialSourcePath` keep the freshest source material attached to that recommendation and `queuedProfiles[*].paths` plus `queuedProfiles[*].latestMaterialSourcePath` keep the same concrete draft files and newest source artifact attached to each queued refresh entry
