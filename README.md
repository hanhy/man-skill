...
- `foundation.core.memory.legacyShortTermSourceCount` and `foundation.core.memory.legacyShortTermSources` preserve which checked-in `memory/short-term/` files were folded into the canonical `daily` lane, so repo-core summaries keep provenance instead of only the canonicalized bucket counts
...
- `memorySummary.legacyShortTermSourceCount` and `memorySummary.legacyShortTermSources` mirror that provenance at the top level, so ingestion/work-loop consumers can tell which legacy short-term files were loaded without reopening the repo tree
