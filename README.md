...
This creates a profile-specific material structure under `profiles/<person-id>/` and exposes a repo summary that can later feed the learning/update layer. You can seed target-person metadata with `update profile`, via a manifest `profiles` block, or via the single-target manifest shorthand, then regenerate derived memory / voice / soul / skills drafts with `--refresh-foundation` or `update foundation --stale`. The repo ships a starter manifest at `samples/harry-materials.json` plus a checked-in text sample at `samples/harry-post.txt`, returns actionable `profileSummaries` after manifest imports, and keeps the top-level `ingestion` block visible even in empty repos so the import/update entrance stays obvious. That block now advertises both a real `sampleManifestCommand` and a one-shot `sampleTextCommand`, while the top-level `workLoop` queue carries those same sample asset paths when ingestion is the current focus. The repo summary also exposes per-profile draft freshness, a repo-level `foundation` rollup, and `foundation.core` diagnostics that now point back to concrete source files for memory buckets, soul, voice, documented skill docs, and placeholder skill folders, so the prompt preview shows both readiness and exactly where to edit next. The top-level `profile`, `memory`, `skills`, and `voice` summaries now also include small readiness-oriented counts like goal totals, identity keys, total memory entries, skill status counts, and voice-guidance counts so downstream planners can reason about the base foundation without reopening the raw files. Delivery planning now follows the same pattern: the top-level `delivery` summary reports auth readiness plus scaffold coverage for Slack, Telegram, WhatsApp, Feishu, OpenAI, Anthropic, Kimi, Minimax, GLM, and Qwen, and links each planned integration to its file under `src/channels/` or `src/models/`. A matching top-level `workLoop` summary now turns those same diagnostics into a priority queue, and the prompt preview/system prompt now mirrors that state in a compact `Work loop:` block so cron-style runs can see the current priority, command, and file paths without re-reading the raw JSON summary.
