...
The top-level `workLoop` summary exposes the queue from a few complementary angles instead of forcing callers to infer intent from one field: `leadingPriority` keeps the first lane in the configured order even when it is already ready, `currentPriority` keeps the queued-or-blocked lane that is actively holding progress, `runnablePriority` keeps the first still-runnable step in priority order even when that step is a ready follow-up such as imported starter-manifest edits, `actionableReadyPriority` keeps the narrower ready-only advisory alias, and `recommendedPriority` stays a stable best-next-action alias that prefers runnable work before falling back to the current blocker or the already-ready leader. The same object also exposes split readiness counts through `readyPriorityCount`, `queuedPriorityCount`, and `blockedPriorityCount`, and each priority can carry additive follow-up surfaces like `fallbackCommand`, `editPath`, `editPaths`, `inspectCommand`, and `followUpCommand` when the next slice includes an edit-first contract instead of one immediate shell command. The prompt preview mirrors that distinction with separate `lead:`, `current:`, `runnable:`, and `advisory:` lines when they differ, plus explicit `fallback:`, `edit:`, `edit paths:`, `inspect after editing:`, and `then run:` lines whenever a priority has post-edit guidance.
...
- when grouped repo skills are present, `summary.skills.categoryCounts`, `foundation.core.skills.categoryCounts`, and `foundation.core.skills.documentedCategoryCounts` expose how many checked-in skills live under each category so delivery-heavy repos can distinguish `channels/*` and `providers/*` coverage without scraping the prompt preview; the top-level `Skill registry:` block now mirrors both as `- categories: ...` and `- documented categories: ...` so undocumented gaps stay visible in the compact operator-facing preview too
