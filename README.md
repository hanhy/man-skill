...
- `foundation.core.soul.shadowPaths` keeps the canonical `SOUL.md` source of truth explicit when shadow docs like `soul/README.md` also exist, so operators can tell which root file is authoritative before editing duplicate soul guidance
...
- `foundation.core.voice.shadowPaths` keeps the canonical `voice/README.md` source of truth explicit when shadow docs like `VOICE.md` also exist, so duplicate voice roots stay inspectable without stealing ownership from the checked-in runtime guide
- `foundation.core.memory.shadowPaths` keeps the canonical `memory/README.md` source of truth explicit when shadow docs like `MEMORY.md` also exist, and `foundation.core.skills.shadowPaths` does the same for the canonical `skills/README.md` source of truth when shadow docs like `SKILLS.md` appear beside the shared skill registry
