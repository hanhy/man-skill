...
- the default checked-in skill catalog stays explicit: 4 channel guides (`channels/feishu`, `channels/slack`, `channels/telegram`, `channels/whatsapp`), 2 profile-ingestion/update guides (`profiles/intake`, `profiles/update`), 6 provider guides (`providers/anthropic`, `providers/glm`, `providers/kimi`, `providers/minimax`, `providers/openai`, `providers/qwen`), plus `cron` and `foundation-maintenance`
- when grouped repo skills are present, `summary.skills.categoryCounts` and `foundation.core.skills.categoryCounts` make that catalog machine-readable by category (`channels`, `profiles`, `providers`, `root`) instead of forcing prompt-only parsing
...
- when grouped repo skills are present, `summary.skills.categoryCounts`, `summary.skills.foundationStatusCounts`, `foundation.core.skills.categoryCounts`, and `foundation.core.skills.documentedCategoryCounts` expose how many checked-in skills live under each category and how many sit in each readiness bucket, so delivery-heavy repos can distinguish `channels/*`, `profiles/*`, and `providers/*` coverage without scraping the prompt preview; the top-level `Skill registry:` block now mirrors those readiness counts as `- foundation statuses: ...` plus the grouped-category lines `- categories: ...` and `- documented categories: ...` so undocumented gaps stay visible in the compact operator-facing preview too
...
- `foundation.core.soul.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` keep the canonical `SOUL.md` source of truth explicit when shadow docs like `soul/README.md` also exist, and mirror the compact first-three-plus-overflow duplicate-doc contract so JSON consumers can reuse the same preview without reparsing prompt text
...
- `foundation.core.voice.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` keep the canonical `voice/README.md` source of truth explicit when shadow docs like `VOICE.md` also exist, so the duplicate-root preview stays machine-readable for voice consumers too
- `foundation.core.memory.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` keep the canonical `memory/README.md` source of truth explicit when shadow docs like `MEMORY.md` also exist, and `foundation.core.skills.shadowPaths`, `shadowPathCount`, `shadowPathSamplePaths`, and `shadowPathOverflowCount` do the same for the canonical `skills/README.md` source of truth when shadow docs like `SKILLS.md` appear beside the shared skill registry, so operators and JSON consumers share the same compact duplicate-doc contract
- the top-level `Soul profile:` and `Voice profile:` preview blocks now also mirror `foundation.core.soul|voice.rootExcerpt` / `rootPath`, section readiness, optional heading aliases, and any surfaced shadow docs, so legacy or duplicate root headings stay operator-visible before the deeper `Core foundation:` diagnostics
...
When the work loop points at one of those thin repo-core repairs, `workLoop.currentPriority.shadowPathCount`, `workLoop.currentPriority.shadowPathSamplePaths`, and `workLoop.currentPriority.shadowPathOverflowCount` (with the same fields mirrored on `workLoop.recommendedPriority` when that repair is the recommendation) now sit beside any full `shadowPaths` list, so downstream tooling can reuse the same first-three-plus-overflow duplicate-doc preview the prompt shows even if only the sampled shadow-doc paths survive.

Even when foundation is already `ready`, `workLoop.leadingPriority.rootHeadingAliases` now keeps the aggregated memory / skills / soul / voice compatibility labels (`what-lives-here->what-belongs-here`, `core-values->core-truths`, `voice-should-capture->signature-moves`, and so on) attached to the same ready-state foundation lane, so legacy root headings stay inspectable before a thin-doc repair is queued.
